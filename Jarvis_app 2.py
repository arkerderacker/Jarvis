...
import os, re, json, datetime, base64, uuid, subprocess, shlex, time, zipfile
...
# -----------------------------------------------------------------------------
# Firebase / Firestore persistent memory
# Uses the Firebase service-account JSON from the Render environment.  This is
# deliberately implemented with the Firestore REST API so app.py remains the
# only file that needs changing.
# -----------------------------------------------------------------------------
FIREBASE_SERVICE_ACCOUNT_ENV = "FIREBASE_SERVICE_ACCOUNT"
FIRESTORE_DATABASE_ID = os.getenv("FIRESTORE_DATABASE_ID", "(default)")
FIRESTORE_FREE_BYTES = 1024 ** 3
...
_firebase_token = {"access_token": None, "expires_at": 0.0}
_firebase_account_cache = None

def _load_firebase_service_account():
    global _firebase_account_cache
    if _firebase_account_cache is not None:
        return _firebase_account_cache
    raw = os.getenv(FIREBASE_SERVICE_ACCOUNT_ENV, "").strip()
    if not raw:
        return None
    try:
        data = json.loads(raw)
        if not isinstance(data, dict) or not data.get("project_id") or not data.get("client_email") or not data.get("private_key"):
            return None
        _firebase_account_cache = data
        return data
    except Exception:
        return None

def firebase_project_id():
    return (_load_firebase_service_account() or {}).get("project_id", "")

def firebase_configured():
    return bool(_load_firebase_service_account())

def _b64url(data):
    return base64.urlsafe_b64encode(data).rstrip(b"=").decode("ascii")

def _firebase_access_token():
    account = _load_firebase_service_account()
    if not account:
        return None
    now = time.time()
    if _firebase_token.get("access_token") and now < _firebase_token.get("expires_at", 0) - 60:
        return _firebase_token["access_token"]
    ...
    try:
        # Use a short-lived temporary file for the service-account private key.
        key_path = os.path.join(JARVIS_DIR, ".firebase_signing_key.pem")
        with open(key_path, "w", encoding="utf-8") as f:
            f.write(account["private_key"])
        try:
            proc = subprocess.run(
                ["openssl", "dgst", "-sha256", "-sign", key_path],
                input=unsigned, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False, timeout=10
            )
        finally:
        ...
        if not token:
            return None
        _firebase_token["access_token"] = token
        _firebase_token["expires_at"] = now + int(token_data.get("expires_in", 3600))
        return token
    except Exception:
        return None

def _firestore_base_url():
    project = firebase_project_id()
    if not project:
        return None
    db = requests.utils.quote(FIRESTORE_DATABASE_ID, safe="()")
    return f"https://firestore.googleapis.com/v1/projects/{project}/databases/{db}/documents"

def _firestore_request(method, path="", json_body=None, params=None, timeout=25):
    ...
    try:
        rr = requests.request(method, url, headers={"Authorization":f"Bearer {token}","Content-Type":"application/json"}, json=json_body, params=params, timeout=timeout)
        if rr.status_code == 401:
            _firebase_token["access_token"] = None
            token = _firebase_access_token()
            if token:
                rr = requests.request(method, url, headers={"Authorization":f"Bearer {token}","Content-Type":"application/json"}, json=json_body, params=params, timeout=timeout)
        return rr
...
def _fs_value(value):
    ...
    if isinstance(value, bytes): return {"bytesValue": base64.b64encode(value).decode("ascii")}
    if isinstance(value, datetime.datetime): return {"timestampValue": value.astimezone(datetime.timezone.utc).isoformat().replace("+00:00","Z")}
    if isinstance(value, list): return {"arrayValue":{"values":[_fs_value(v) for v in value]}}
    if isinstance(value, dict): return {"mapValue":{"fields":{str(k):_fs_value(v) for k,v in value.items()}}}
    return {"stringValue": str(value)}

def _from_fs_value(v):
    ...
    if "bytesValue" in v:
        try: return base64.b64decode(v["bytesValue"])
        except Exception: return b""
    if "arrayValue" in v: return [_from_fs_value(x) for x in v.get("arrayValue",{}).get("values",[])]
    if "mapValue" in v: return {k:_from_fs_value(x) for k,x in v.get("mapValue",{}).get("fields",{}).items()}
    return None

def _fs_doc(data):
    return {"fields":{str(k):_fs_value(v) for k,v in data.items()}}

def _fs_doc_data(doc):
    out={k:_from_fs_value(v) for k,v in doc.get("fields",{}).items()}
    out["_document_name"] = doc.get("name", "")
    return out

def _fs_set(collection, doc_id, data):
    doc_id = requests.utils.quote(str(doc_id), safe="-_.~")
    rr = _firestore_request("PATCH", f"{collection}/{doc_id}", _fs_doc(data))
    return bool(rr is not None and rr.ok)

def _fs_delete(collection, doc_id):
    doc_id = requests.utils.quote(str(doc_id), safe="-_.~")
    rr = _firestore_request("DELETE", f"{collection}/{doc_id}")
    return bool(rr is not None and rr.ok)

def _fs_list(collection, page_size=1000):
    if not firebase_configured(): return []
    out=[]; token=None
    while True:
        params={"pageSize":min(page_size,1000)}
        if token: params["pageToken"]=token
        rr=_firestore_request("GET", collection, params=params)
        if rr is None or not rr.ok: break
        data=rr.json();out.extend(_fs_doc_data(d) for d in data.get("documents",[]))
        token=data.get("nextPageToken")
        if not token: break
    return out

def _fs_delete_collection(collection):
    docs=_fs_list(collection, page_size=1000)
    ok=True
    for doc in docs:
        name=doc.get("_document_name","")
        doc_id=name.rsplit("/",1)[-1] if name else ""
        if doc_id and not _fs_delete(collection,doc_id): ok=False
    return ok

def _firebase_message_id(): return f"{time.time_ns()}_{uuid.uuid4().hex[:10]}"

def firebase_save_message(role, content, source="chat", extra=None):
    if not firebase_configured(): return False
    ...
    return _fs_set(FIRESTORE_CONVERSATIONS,_firebase_message_id(),data)

def firebase_save_memory(user_text, assistant_text, source="conversation"):
    if not firebase_configured(): return False
    text=f"User: {user_text}\nJarvis: {assistant_text}"
    return _fs_set(FIRESTORE_MEMORIES,_firebase_message_id(),{
        "text":text,"user_text":str(user_text),"assistant_text":str(assistant_text),"source":source,
...
def firebase_relevant_memories(query, limit=6):
    ...
    if not q: return []
    scored=[]
    for item in _fs_list(FIRESTORE_MEMORIES, page_size=1000):
        text=item.get("text","")
        overlap=len(q & _memory_tokens(text))
        if overlap:
            recency=item.get("created_at","")
            scored.append((overlap,recency,text))
    scored.sort(key=lambda x:(x[0],x[1]), reverse=True)
    return [x[2] for x in scored[:limit]]

def firebase_load_messages():
    if not firebase_configured(): return []
    docs=_fs_list(FIRESTORE_CONVERSATIONS, page_size=1000)
    docs=[d for d in docs if d.get("role") in ("user","assistant") and d.get("content") is not None]
    docs.sort(key=lambda d:d.get("created_at", ""))
    return [{"role":d["role"],"content":d["content"]} for d in docs]

def firebase_clear_memory():
    if not firebase_configured(): return True
    ok1=_fs_delete_collection(FIRESTORE_CONVERSATIONS)
    ok2=_fs_delete_collection(FIRESTORE_MEMORIES)
    return ok1 and ok2

def firebase_estimated_bytes():
    if not firebase_configured(): return None
    total=0
    def _size(value):
        if isinstance(value, bytes): return len(value)
        if isinstance(value, dict): return sum(_size(k)+_size(v) for k,v in value.items())
        if isinstance(value, list): return sum(_size(v) for v in value)
        return len(str(value).encode("utf-8"))
    for collection in (FIRESTORE_CONVERSATIONS,FIRESTORE_MEMORIES,FIRESTORE_PHOTOS,FIRESTORE_PHOTO_CHUNKS):
        for item in _fs_list(collection, page_size=1000):
            total += _size({k:v for k,v in item.items() if k != "_document_name"})
    return total

def firebase_usage():
    used=firebase_estimated_bytes()
    if used is None: return {"configured":False,"used_bytes":0,"remaining_bytes":FIRESTORE_FREE_BYTES,"quota_bytes":FIRESTORE_FREE_BYTES}
    return {"configured":True,"used_bytes":used,"remaining_bytes":max(0,FIRESTORE_FREE_BYTES-used),"quota_bytes":FIRESTORE_FREE_BYTES}

def firebase_save_photo(data, filename, mime_type):
    if not firebase_configured(): return None
    photo_id=_firebase_message_id()
    raw=bytes(data); chunk_size=650000
    meta={"filename":filename or "photo.jpg","mime_type":mime_type or "image/jpeg","size_bytes":len(raw),"created_at":datetime.datetime.now(datetime.timezone.utc).isoformat().replace("+00:00","Z"),"chunks":(len(raw)+chunk_size-1)//chunk_size}
    if not _fs_set(FIRESTORE_PHOTOS,photo_id,meta): return None
    for i in range(meta["chunks"]):
        chunk=raw[i*chunk_size:(i+1)*chunk_size]
        if not _fs_set(FIRESTORE_PHOTO_CHUNKS,f"{photo_id}_{i:05d}",{"photo_id":photo_id,"index":i,"data":chunk}):
            return None
    return photo_id

def firebase_load_photo(photo_id):
    if not firebase_configured(): return None
    docs=_fs_list(FIRESTORE_PHOTO_CHUNKS, page_size=1000)
    chunks=[d for d in docs if d.get("photo_id")==photo_id]
    chunks.sort(key=lambda d:int(d.get("index",0)))
    if not chunks: return None
    raw=b"".join(d.get("data",b"") for d in chunks)
    meta_docs=[d for d in _fs_list(FIRESTORE_PHOTOS, page_size=1000) if d.get("_document_name","").endswith("/"+photo_id)]
    meta=meta_docs[0] if meta_docs else {}
    return raw, meta.get("mime_type","image/jpeg")

def firebase_list_photos():
    if not firebase_configured(): return []
    out=[]
    for d in _fs_list(FIRESTORE_PHOTOS,page_size=1000):
        name=d.get("_document_name","").rsplit("/",1)[-1]
        if name: out.append({"name":name,"filename":d.get("filename",name),"url":"/api/photos/file/"+name,"created_at":d.get("created_at","")})
    out.sort(key=lambda x:x.get("created_at",""), reverse=True)
    return out

def firebase_delete_photo(photo_id):
    if not firebase_configured(): return False
    chunks=[d for d in _fs_list(FIRESTORE_PHOTO_CHUNKS,page_size=1000) if d.get("photo_id")==photo_id]
    ok=True
    for d in chunks:
        name=d.get("_document_name","").rsplit("/",1)[-1]
        if name and not _fs_delete(FIRESTORE_PHOTO_CHUNKS,name): ok=False
    if not _fs_delete(FIRESTORE_PHOTOS,photo_id): ok=False
    return ok

def load_chat():
    if firebase_configured():
        # An empty remote history is authoritative after the user clears memory.
        return firebase_load_messages()
...
def save_chat(items):
    # Firestore is the source of truth. The local file is only a small offline
    # cache and is never allowed to delete/trim the remote history.
...
def is_configured(): return bool(runtime_cfg.get("groq_key"))

def ask_groq(history):
    key=runtime_cfg.get("groq_key")
    if not key: return "I don't have a Groq API key yet - please add it in setup."
    try:
        r=requests.post(GROQ_URL,headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"},
                        json={"model":GROQ_MODEL,"messages":history,"temperature":0.7,"max_tokens":700},timeout=60)
        record_groq_usage(r)
        if r.status_code!=200: return f"I couldn't reach Groq right now (HTTP {r.status_code})."
        return r.json()["choices"][0]["message"]["content"].strip()
    except Exception as e: return f"I ran into a network error talking to Groq: {e}"
...
def handle_command(text):
    ...
    if lower in ("clear chat","reset","clear conversation"):
        firebase_clear_memory()
        conversation_history[:]=[conversation_history[0]]
        save_chat([])
        return "Cleared - what would you like to talk about?",None
    ...
    if topic_match:
        topic=topic_match.group(1).strip();web_context=web_search(topic)
        user_context=f"Investigate this topic: {topic}. Web context: {web_context}"
        relevant=firebase_relevant_memories(topic, limit=4)
        history=[conversation_history[0]]
        if relevant: history.append({"role":"system","content":"Relevant older Jarvis memory:\n\n"+"\n\n---\n\n".join(relevant)})
        history.extend(conversation_history[1:]);history.append({"role":"user","content":user_context})
        reply=ask_groq(history)
        conversation_history.append({"role":"user","content":user_context});conversation_history.append({"role":"assistant","content":reply})
        firebase_save_message("user",user_context,"web")
        firebase_save_message("assistant",reply,"web")
        firebase_save_memory(user_context,reply,"web")
        return reply,None
    relevant=firebase_relevant_memories(stripped, limit=6)
    prompt_history=[conversation_history[0]]
    if relevant:
        prompt_history.append({"role":"system","content":"Relevant long-term Jarvis memory from older conversations:\n\n"+"\n\n---\n\n".join(relevant)})
    prompt_history.extend(conversation_history[1:])
    prompt_history.append({"role":"user","content":stripped})
    reply=ask_groq(prompt_history)
    conversation_history.append({"role":"user","content":stripped})
    conversation_history.append({"role":"assistant","content":reply})
    firebase_save_message("user",stripped)
    firebase_save_message("assistant",reply)
    firebase_save_memory(stripped,reply)
    save_chat([m for m in conversation_history[1:] if m.get("role") in ("user","assistant")])
    # Only the in-memory prompt window is trimmed. Firestore keeps the complete history.
...
def api_setup():
    ...
    return jsonify({"ok":True})
def _youtube_first_real_video(html):
    """Return the first normal YouTube video result, skipping promoted results.

    YouTube's result data is embedded in a large, nested JSON blob whose exact
    script wrapper changes frequently.  Looking specifically for normal
    videoRenderer entries is more robust than trying to parse the entire page
    with a single greedy/non-greedy JSON regex.
    """
    if not html:
        return None

    # Normal search results use videoRenderer. Promoted/sponsored results use
    # promotedVideoRenderer and are deliberately ignored.
    pattern = re.compile(
        r'"videoRenderer"\s*:\s*\{.{0,5000}?"videoId"\s*:\s*"([A-Za-z0-9_-]{11})"',
        re.S,
    )
    for match in pattern.finditer(html):
        # Sponsored results use promotedVideoRenderer, not videoRenderer, so
        # every match here is already a normal result renderer.
...
def api_youtube_first():
    ...
    try:
        r=requests.get(search_url,headers={"User-Agent":"Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 Version/17.0 Mobile/15E148 Safari/604.1","Accept-Language":"en-US,en;q=0.9"},timeout=15)
        if r.ok:
            vid=_youtube_first_real_video(r.text)
            if vid:
                return jsonify({"video_id":vid,"url":f"https://www.youtube.com/watch?v={vid}","fallback":search_url})
    except requests.RequestException:
        pass
...
def api_photo():
    ...
    user_text=question or "I uploaded a photo for you to inspect."
    saved_name=data.get("saved_name") or ""
    photo_id=saved_name
    if firebase_configured() and not photo_id:
        try:
            header,payload=data_url.split(",",1)
            raw=base64.b64decode(payload)
            mime=(header.split(";",1)[0].split(":",1)[-1] or "image/jpeg")
            photo_id=firebase_save_photo(raw,"analysis.jpg",mime) or ""
        except Exception:
            photo_id=""
    conversation_history.append({"role":"user","content":"[Photo] "+user_text})
    conversation_history.append({"role":"assistant","content":reply})
    extra={"photo_id":photo_id} if photo_id else None
    firebase_save_message("user","[Photo] "+user_text,"photo",extra)
    firebase_save_message("assistant",reply,"photo",extra)
    firebase_save_memory("[Photo] "+user_text,reply,"photo")
    save_chat([m for m in conversation_history[1:] if m.get("role") in ("user","assistant")])
    if len(conversation_history)>21: conversation_history[:]=[conversation_history[0]]+conversation_history[-20:]
    return jsonify({"reply":reply,"saved_name":photo_id})

@app.route("/api/chat")
def api_chat():
    return jsonify({"messages":load_chat(),"firebase_configured":firebase_configured()})
...
def api_photos():
    if firebase_configured():
        return jsonify({"photos":firebase_list_photos(),"storage":"firestore"})
    photos=[]
    ...
    return jsonify({"photos":photos,"storage":"local"})
...
def api_photo_upload():
    ...
    if firebase_configured():
        photo_id=firebase_save_photo(data,(photo.filename or "photo"),photo.mimetype or "image/jpeg")
        if not photo_id:return jsonify({"error":"Firebase memory is configured, but the photo could not be saved."}),502
        return jsonify({"ok":True,"name":photo_id,"url":"/api/photos/file/"+photo_id,"storage":"firestore"})
    ...
    return jsonify({"ok":True,"name":name,"url":"/api/photos/file/"+name,"storage":"local"})

@app.route("/api/photos/file/<path:name>")
def api_photo_file(name):
    if firebase_configured():
        loaded=firebase_load_photo(os.path.basename(name))
        if not loaded:return jsonify({"error":"Photo not found."}),404
        raw,mime=loaded
...
def api_photo_delete():
    ...
    if firebase_configured():
        if not firebase_delete_photo(name):return jsonify({"error":"Photo not found or could not be deleted."}),404
        return jsonify({"ok":True})
    path=os.path.join(PHOTOS_DIR,name)
    if not os.path.isfile(path):return jsonify({"error":"Photo not found."}),404
    os.remove(path);return jsonify({"ok":True})

@app.route("/api/firebase/usage")
def api_firebase_usage():
    return jsonify(firebase_usage())
...
def api_usage():
    ...
    return jsonify({"groq":u,"elevenlabs":eleven})
...
$('aboutBtn').onclick=async()=>{$('aboutPanel').classList.add('open');$('settings').classList.remove('open');$('library').classList.remove('open');try{const r=await fetch('/api/firebase/usage');const d=await r.json();if(d.configured){const used=(d.used_bytes||0),left=(d.remaining_bytes||0);const fmt=n=>n<1024?(n+' B'):n<1024*1024?((n/1024).toFixed(1)+' KB'):((n/1024/1024).toFixed(2)+' MB');$('firebaseDataUsage').textContent=fmt(used)+' used · '+fmt(left)+' remaining (1 GiB Firestore quota)'}else $('firebaseDataUsage').textContent='Not configured'}catch(e){$('firebaseDataUsage').textContent='Unavailable'}};
...
$('clearMemory').onclick=async()=>{if(!confirm('Clear saved conversation memory?'))return;const r=await fetch('/api/message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:'clear chat'})});if(!r.ok){alert('Could not clear Firebase memory.');return}messages.innerHTML='';await loadSavedChat();addMsg('j','Cleared - what would you like to talk about?')};
