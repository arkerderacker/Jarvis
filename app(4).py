...
def _provider_probe(provider):
 key=runtime_cfg.get(AI_PROVIDERS[provider]["key"])
 if not key:return False,"missing"
 try:
  if provider=="gemini":
   r=requests.post(
    f"https://generativelanguage.googleapis.com/v1beta/models/{AI_PROVIDERS['gemini']['model']}:generateContent",
    params={"key":key},
    json={"contents":[{"role":"user","parts":[{"text":"Reply with OK."}]}],"generationConfig":{"maxOutputTokens":2,"temperature":0}},
    timeout=15)
  else:
   headers={"Authorization":f"Bearer {key}","Content-Type":"application/json"}
   if provider=="openrouter":headers.update({"HTTP-Referer":"https://jarvis.local","X-Title":"Jarvis"})
   r=requests.post(AI_PROVIDERS[provider]["url"],headers=headers,json={"model":AI_PROVIDERS[provider]["model"],"messages":[{"role":"user","content":"Reply with OK."}],"temperature":0,"max_tokens":2},timeout=15)
  if r.status_code in (401,403):return False,"invalid_key"
  if r.status_code in (402,429):return False,"quota"
  if not r.ok:return False,f"http_{r.status_code}"
  return True,"ok"
 except requests.RequestException:return False,"network"
...
def api_ai_providers():
    if request.method=="GET":
        ...
        for k,v in AI_PROVIDERS.items():
            configured=bool(runtime_cfg.get(v["key"]))
            status=runtime_cfg.get(f"{k}_provider_status","saved" if configured else "not_configured")
            if verify and configured:
                ok,status=_provider_probe(k);runtime_cfg[f"{k}_provider_status"]=status
                if ok:runtime_cfg[f"{k}_provider_verified_at"]=time.time()
                save_config(runtime_cfg)
            statuses.append({"id":k,"label":v["label"],"configured":configured,"verified":status=="ok","status":status,"model":v["model"]})
        return jsonify({"default":"groq","selected":runtime_cfg.get("ai_provider") or "groq","active":runtime_cfg.get("last_ai_provider") or runtime_cfg.get("ai_provider") or "groq","providers":statuses})
    ...
    old_key=runtime_cfg.get(AI_PROVIDERS[provider]["key"])
    if supplied_key:runtime_cfg[AI_PROVIDERS[provider]["key"]]=supplied_key
    elif not old_key:return jsonify({"error":"Enter an API key for this provider first."}),400
    ok,status=_provider_probe(provider)
    if not ok:
        if supplied_key:
            if old_key:runtime_cfg[AI_PROVIDERS[provider]["key"]]=old_key
            else:runtime_cfg.pop(AI_PROVIDERS[provider]["key"],None)
        runtime_cfg[f"{provider}_provider_status"]=status;save_config(runtime_cfg)
        message={"invalid_key":"The API key was rejected by this provider.","quota":"The provider accepted the key but its quota/credits are unavailable.","network":"Could not reach the provider.","missing":"No API key is configured."}.get(status,f"Provider check failed ({status}).")
        return jsonify({"ok":False,"error":message,"provider":provider,"status":status,"selected":runtime_cfg.get("ai_provider") or "groq"}),502
    runtime_cfg[f"{provider}_provider_status"]="ok"
    runtime_cfg[f"{provider}_provider_verified_at"]=time.time()
    runtime_cfg["ai_provider"]=provider
    runtime_cfg["last_ai_provider"]=provider
    save_config(runtime_cfg)
    return jsonify({"ok":True,"selected":provider,"active":provider,"configured":True,"verified":True,"status":"ok","model":AI_PROVIDERS[provider]["model"]})
