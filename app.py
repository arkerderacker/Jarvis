...
#immersivePointer{position:absolute;z-index:5060;width:22px;height:22px;border:2px solid var(--accent);border-radius:50%;pointer-events:none;display:none;transform:translate(-50%,-50%);box-shadow:0 0 22px var(--accent),inset 0 0 8px color-mix(in srgb,var(--accent) 35%,transparent)}
...
#jarvisImmersive .jiMenu{z-index:5070}
#jarvisImmersive .jiTop,#jarvisImmersive #jarvisVisionStatus{z-index:5070}
...
#jarvisDraw{position:fixed!important;left:0!important;top:0!important;width:100vw!important;height:100vh!important;inset:auto!important;touch-action:none;pointer-events:none}
...
(()=>{
 ...
 let immersive=false, fistStart=0, lastToggle=0, drawColor=localStorage.getItem('jarvisDrawColor')||'#46e0d0', brushSize=4;
 ...
 function isPointing(h){return !fingerCurled(h,8,6)&&fingerCurled(h,12,10)&&fingerCurled(h,16,14)&&fingerCurled(h,20,18)}
 ...
 window.jarvisHandleHands=function(input){const hands=Array.isArray(input?.[0])?input:[];if(!hands.length){if($('pointer'))$('pointer').style.display='none';if(immersive){fistStart=0;if(countdown)countdown.classList.remove('show')}return}
   const h=hands[0];if(!immersive){normalHand(h);if(isFist(h)){if(!fistStart)fistStart=performance.now();const elapsed=performance.now()-fistStart,remain=Math.max(0,5-Math.floor(elapsed/1000));if(countdown){countdown.innerHTML='<b>'+remain+'</b>KEEP FIST CLOSED · IMMERSIVE';countdown.classList.add('show')}if(elapsed>=5000){enter()}}else{fistStart=0;if(countdown)countdown.classList.remove('show')}return}
