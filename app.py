...
<script>
/* New Jarvis interaction layer. It hooks into the existing camera/voice/chat systems. */
(()=> {
 ...
 let immersive=false, fistSince=0, lastModeToggle=0, drawColor=localStorage.getItem('jarvisDrawColor')||'#46e0d0';
 let drawing=false, pinchMove=false, strokes=[], currentStroke=null, selectedStroke=null, lastDraw=null;
 let panelDrag=null, uiOffset={};
 const colors=['#46e0d0','#59a7ff','#a97cff','#ffb84d','#ff5f7a','#67e48b'];
 function makeWave(){if(!wave)return;wave.innerHTML='';for(let i=0;i<28;i++){let b=document.createElement('i');b.dataset.i=i;wave.appendChild(b)}}
 makeWave();
 function setState(state){
   if(!core)return;core.className=state;
   const status=$('status');if(status)status.textContent=state.toUpperCase();
   document.querySelectorAll('#jarvisTools .jt').forEach(x=>x.classList.remove('active'));
   const map={listening:'mic',thinking:'web',speaking:'mic'};
   if(map[state])document.querySelector(`#jarvisTools .jt[data-tool="${map[state]}"]`)?.classList.add('active');
 }
 function activity(label,text,tool){
   ...
   const el=document.createElement('div');el.className='jaItem';el.innerHTML='<b>'+String(label).replace(/[<>&]/g,'')+'</b>'+String(text).replace(/[<>&]/g,'');
   box.prepend(el);while(box.children.length>4)box.lastElementChild.remove();
 }
 function showTip(text){const t=$('jarvisTip');if(!t)return;t.textContent='JARVIS · '+text;t.classList.add('show');clearTimeout(t._tm);t._tm=setTimeout(()=>t.classList.remove('show'),6500)}
 ...
 function canvasPoint(ev){const r=$('jarvisDraw').getBoundingClientRect();return {x:(ev.clientX-r.left)/r.width,y:(ev.clientY-r.top)/r.height}}
 function redraw(){
   const c=$('jarvisDraw'),ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);
   for(const st of strokes){if(st.points.length<2)continue;ctx.strokeStyle=st.color;ctx.lineWidth=st.size*Math.max(devicePixelRatio,1);ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();st.points.forEach((p,i)=>{const x=p.x*c.width,y=p.y*c.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke()}
 }
 function resizeCanvas(){const c=$('jarvisDraw');if(!c)return;const d=Math.min(devicePixelRatio||1,2),w=innerWidth,h=innerHeight,old=document.createElement('canvas');if(c.width&&c.height){old.width=c.width;old.height=c.height;old.getContext('2d').drawImage(c,0,0)}c.width=w*d;c.height=h*d;c.style.width=w+'px';c.style.height=h+'px';if(old.width){}redraw()}
 addEventListener('resize',()=>{if(immersive)resizeCanvas()});
 function enterImmersive(){
   if(immersive||performance.now()-lastModeToggle<1800)return;
   immersive=true;lastModeToggle=performance.now();fistSince=0;imm.classList.add('open');imm.setAttribute('aria-hidden','false');
   const src=$('handcam');const iv=$('jarvisImmersiveVideo');if(src&&src.srcObject)iv.srcObject=src.srcObject;
   resizeCanvas();activity('IMMERSIVE','Entering immersive mode.','vision');setState('thinking');showTip('Entering immersive mode.');try{window.speechSynthesis?.cancel()}catch(e){}
   if(typeof speakJarvis==='function')speakJarvis('Entering immersive mode.');
   setTimeout(()=>setState('listening'),900);
 }
 function exitImmersive(){
   if(!immersive||performance.now()-lastModeToggle<800)return;
   immersive=false;lastModeToggle=performance.now();imm.classList.remove('open');imm.setAttribute('aria-hidden','true');drawing=false;pinchMove=false;fistSince=0;
   activity('IMMERSIVE','Exiting immersive mode.','vision');setState('idle');showTip('Immersive mode closed.');if(typeof speakJarvis==='function')speakJarvis('Exiting immersive mode.');
 }
 window.jarvisEnterImmersive=enterImmersive;window.jarvisExitImmersive=exitImmersive;
 $('jarvisExit').onclick=exitImmersive;
 $('jarvisClear').onclick=()=>{strokes=[];selectedStroke=null;redraw();activity('DRAW','Canvas cleared.','vision')};
 $('jarvisUndo').onclick=()=>{strokes.pop();selectedStroke=null;redraw()};
 async function saveCreation(){
   if(!strokes.length){showTip('There is nothing to save yet.');return}
   let name=prompt('What would you like to name your creation?');if(!name)return;
   const r=await fetch('/api/creations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,strokes})});
   const d=await r.json();if(!r.ok)throw Error(d.error||'Could not save creation.');
   activity('MEMORY','Saved “'+name+'”.','memory');showTip('Saved creation: '+name);
 }
 async function loadCreation(){
   const name=prompt('Which creation should I load?');if(!name)return;
 ...
 $('jarvisSave').onclick=()=>saveCreation().catch(e=>showTip(e.message));
 $('jarvisLoad').onclick=()=>loadCreation().catch(e=>showTip(e.message));
 const c=$('jarvisDraw');
 c.addEventListener('pointerdown',e=>{if(!immersive)return;const p=canvasPoint(e);drawing=true;currentStroke={color:drawColor,size:4,points:[p]};strokes.push(currentStroke);c.setPointerCapture?.(e.pointerId)});
 c.addEventListener('pointermove',e=>{if(!immersive||!drawing||!currentStroke)return;currentStroke.points.push(canvasPoint(e));redraw()});
 c.addEventListener('pointerup',()=>{drawing=false;currentStroke=null});
 function strokeHit(p){
   let best=null,bd=.035;
   for(const st of strokes){for(const q of st.points){const d=Math.hypot(q.x-p.x,q.y-p.y);if(d<bd){bd=d;best=st;break}}}
   return best;
 }
 window.jarvisHandleHands=function(h){
   if(!h)return;
   const idx=h[8],mid=h[12],ring=h[16],pinky=h[20],thumb=h[4],wrist=h[0];
   const ix=1-idx.x, iy=idx.y;
   const curled=(Math.hypot(idx.x-wrist.x,idx.y-wrist.y)<Math.hypot(mid.x-wrist.x,mid.y-wrist.y)*.86 &&
                 Math.hypot(ring.x-wrist.x,ring.y-wrist.y)<Math.hypot(mid.x-wrist.x,mid.y-wrist.y)*.82 &&
                 Math.hypot(pinky.x-wrist.x,pinky.y-wrist.y)<Math.hypot(mid.x-wrist.x,mid.y-wrist.y)*.78);
   if(!immersive){
     if(curled){if(!fistSince)fistSince=performance.now();if(performance.now()-fistSince>=5000){enterImmersive();return}}else fistSince=0;
     const together=Math.hypot(idx.x-mid.x,idx.y-mid.y)<.065;
     if(together && !panelDrag){
       ...
       if(target){panelDrag={el:target,x:ix,y:iy,l:parseFloat(target.dataset.jx)||0,t:parseFloat(target.dataset.jy)||0}}
     }else if(!together)panelDrag=null;
     if(panelDrag){const dx=(ix-panelDrag.x)*innerWidth,dy=(iy-panelDrag.y)*innerHeight;panelDrag.l+=dx;panelDrag.t+=dy;panelDrag.x=ix;panelDrag.y=iy;panelDrag.el.style.transform=`translate(${panelDrag.l}px,${panelDrag.t}px)`;panelDrag.el.classList.add('jiDragging')}
     else document.querySelectorAll('.jiDragging').forEach(x=>x.classList.remove('jiDragging'));
     return;
   }
   const c=$('jarvisDraw');const rect=c.getBoundingClientRect(),p={x:ix,y:iy};
   ...
   if(pinch){
     if(hit && !selectedStroke){selectedStroke=hit;pinchMove=true;lastDraw=p;activity('DRAW','Grabbed a stroke.','vision')}
     if(pinchMove&&selectedStroke&&lastDraw){const dx=p.x-lastDraw.x,dy=p.y-lastDraw.y;selectedStroke.points.forEach(q=>{q.x=Math.max(0,Math.min(1,q.x+dx));q.y=Math.max(0,Math.min(1,q.y+dy))});lastDraw=p;redraw()}
   }else{selectedStroke=null;pinchMove=false;lastDraw=null}
 };
 // Vision is explicitly on-demand: no frame is uploaded until the user asks.
 window.jarvisVisionRequest=async function(question){
   if(!immersive)return false;
   const v=$('jarvisImmersiveVideo');if(!v||!v.videoWidth)return false;
   const c=document.createElement('canvas');c.width=Math.min(v.videoWidth,1280);c.height=Math.round(c.width*v.videoHeight/v.videoWidth);
   c.getContext('2d').drawImage(v,0,0,c.width,c.height);const image=c.toDataURL('image/jpeg',.78);
   $('jarvisVisionStatus').textContent='VISION ANALYZING';activity('VISION','Analyzing the current frame only.','vision');
   try{const r=await fetch('/api/photo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image,question:question||'What am I holding? Describe what you see in the current frame.'})});const d=await r.json();if(!r.ok)throw Error(d.error||'Vision failed.');$('jarvisVisionStatus').textContent='VISION READY';return d.reply}
   catch(e){$('jarvisVisionStatus').textContent='VISION ERROR';throw e}
 };
 // Smoothly reveal the new UI only after the existing page is usable, avoiding the stuck initialize overlay.
 ...
 // Re-run a bounded status check if the original setup request failed; this does not bypass the API-key requirement.
 async function robustInit(){
   try{
     const r=await Promise.race([fetch('/api/status'),new Promise((_,rej)=>setTimeout(()=>rej(Error('timeout')),8000))]);
     const d=await r.json();
     if(d.configured){$('setup')?.classList.add('hidden');activity('CORE','Jarvis is online.','groq');}
     else $('err') && ($('err').textContent='Add your Groq API key to initialize Jarvis.');
   }catch(e){$('err') && ($('err').textContent='Server connection timed out. Refresh or try Initialize again.');}
 }
 setTimeout(robustInit,1200);
 // Proactive tips: quiet, useful, and never frequent.
 let lastTip=0;
 window.jarvisMaybeTip=function(context){
   if(localStorage.getItem('jarvisProactive')==='off')return;
   if(performance.now()-lastTip<120000)return;
   const tips=['Keep immersive drawing local for responsive hand tracking.','You can ask me what is in the current camera frame without continuously uploading video.','Name a creation after saving it and I can load it again later.'];
   if(context&&/weather|study|code|project|jarvis/i.test(context)){lastTip=performance.now();showTip(tips[Math.floor(Math.random()*tips.length)]);}
 };
 // Middle+index drag moves open panels; refresh resets because offsets are kept only in memory.
 ...
 setInterval(()=>{if(typeof listening!=='undefined'&&listening)setState('listening');else if(typeof isJarvisSpeaking!=='undefined'&&isJarvisSpeaking)setState('speaking');else if(typeof voiceTarget!=='undefined'&&voiceTarget>.18)setState('listening');else if(typeof status!=='undefined'&&status.textContent==='THINKING')setState('thinking');else setState(immersive?'listening':'idle');
   const level=typeof voiceLevel!=='undefined'?voiceLevel:0;
   document.querySelectorAll('#jarvisWave i').forEach((b,i)=>{const dist=Math.abs(i-14)/14;const n=Math.max(.12,(1-dist)*level*(.75+Math.random()*.7)+.12);b.style.height=(n*100)+'%';b.style.transform=`scaleY(${.65+level*1.4})`});
 },70);
 // Intercept normal send only for on-demand immersive vision questions.
 const wrapSend=()=>{if(typeof send!=='function'||window.__jarvisSendWrapped)return;window.__jarvisSendWrapped=true;const oldSend=send;window.send=async function(){
   const textEl=$('text'),q=(textEl?.value||'').trim();
   if(immersive&&/what am i holding|what is this|what's this|what do you see|describe (this|what you see)|identify (this|what)/i.test(q)){
     try{const reply=await window.jarvisVisionRequest(q);if(typeof addMsg==='function')addMsg('j',reply);if(typeof speakJarvis==='function')speakJarvis(reply);if(textEl)textEl.value='';return}catch(e){if(typeof addMsg==='function')addMsg('j',e.message||'I could not analyze the current camera frame.');return}
   }
   const result=oldSend.apply(this,arguments);window.jarvisMaybeTip(q);return result;
 }};
 setTimeout(wrapSend,1500);
})();
</script>
</body></html>'''
