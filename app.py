...
#jarvisImmersive{position:fixed;inset:0;background:#03070b;z-index:5000;display:none;opacity:0;transition:opacity .35s,transform .35s;transform:scale(.985)}
...
#jarvisImmersive video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.32;transform:scaleX(-1)}
#jarvisDraw{position:absolute;inset:0;width:100%;height:100%;touch-action:none}
...
async function startHandsAfterInit(){try{await initHands();$('status').textContent='READY';if(window.jarvisActivity)window.jarvisActivity('VISION','Hand tracking ready.','vision');return true}catch(e){$('status').textContent='CAMERA RETRY';$('err').textContent=e.message||'Camera/hand tracking could not start.';if(window.jarvisActivity)window.jarvisActivity('VISION','Camera/hand tracking unavailable; Jarvis core remains online.','error');return false}}
async function setup(){let r=await fetch('/api/status').catch(()=>null);if(!r)return;let d=await r.json().catch(()=>({}));if(d.configured){$('err').textContent='Initializing Jarvis core…';$('status').textContent='STARTING CORE';$('setup').classList.add('hidden');addMsg('j','Hello. Jarvis core is online. Initializing camera controls…');const ok=await startHandsAfterInit();if(!ok){$('setup').classList.remove('hidden');$('status').textContent='CAMERA RETRY';}else{ $('setup').classList.add('hidden'); addMsg('j','Camera and gesture control are ready.'); }}}
$('begin').onclick=async()=>{let key=$('key').value.trim();if(!key){$('err').textContent='A Groq API key is required.';return}$('begin').disabled=true;$('err').textContent='Connecting to Jarvis…';try{let r=await fetch('/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({groq_key:key,save:$('save').checked})});let d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw Error(d.error||'The server did not accept the Groq key.');$('err').textContent='Groq connected. Starting Jarvis core…';$('status').textContent='STARTING CORE';$('setup').classList.add('hidden');addMsg('j','Jarvis core is online. Starting camera and gesture control…');const ok=await startHandsAfterInit();if(!ok){$('setup').classList.remove('hidden');$('begin').disabled=false;}else{ $('setup').classList.add('hidden'); addMsg('j','Jarvis initialized. Gesture control and chat are ready.'); }}catch(e){$('setup').classList.remove('hidden');$('err').textContent=e.message||'Could not initialize Jarvis.';$('begin').disabled=false}};
...
(()=>{
 ...
 function enter(){if(immersive||performance.now()-lastToggle<1200)return;immersive=true;lastToggle=performance.now();fistStart=0;if(countdown){countdown.classList.remove('show');countdown.textContent=''}imm.classList.add('open','entering');imm.setAttribute('aria-hidden','false');
   const src=$('handcam');if(src?.srcObject){video.srcObject=src.srcObject;video.play?.().catch(()=>{})}
   resizeCanvas();activity('IMMERSIVE','Camera workspace active.','vision');state('listening');setTimeout(()=>imm.classList.remove('entering'),650);ip?.classList.add('on');
 }
 function exit(){if(!immersive||performance.now()-lastToggle<700)return;immersive=false;lastToggle=performance.now();imm.classList.remove('open','entering');imm.setAttribute('aria-hidden','true');drawing=false;currentStroke=null;selectedStroke=null;lastPinch=null;resizeStart=null;resizeBase=null;ip?.classList.remove('on');activity('IMMERSIVE','Returned to normal interface.','vision');state('idle')}
