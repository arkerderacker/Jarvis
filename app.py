...
def _groq_probe():
    ...
    try:
        ...
        # Keep the same usage/rate-limit accounting used by real Groq requests.
        record_groq_usage(r)
        if r.status_code in (401, 403):
            return False, "invalid_key", r
        if r.status_code in (402, 429):
            return False, "quota", r
        if not r.ok:
            return False, f"http_{r.status_code}", r
        return True, "ok", r
    except requests.RequestException:
...
def api_ai_providers():
    # Kept as the existing endpoint so the current UI/API contract still works,
    # but it is intentionally Groq-only.
    if request.method=="GET":
        verify=request.args.get("verify")=="1"
        configured=bool(runtime_cfg.get("groq_key"))
        status=runtime_cfg.get("groq_provider_status","not_configured" if not configured else "not_verified")
        verified=False
        probe_error=None
        if verify and configured:
            ok,status,_resp=_groq_probe()
            verified=ok
            runtime_cfg["groq_provider_status"]=status
            if ok:
                runtime_cfg["groq_provider_verified_at"]=time.time()
            save_config(runtime_cfg)
        else:
            verified=(status=="ok")
        return jsonify({
    ...
    provider=(d.get("provider") or "groq").strip().lower()
    if provider!="groq":
        return jsonify({"error":"Only Groq is supported. Other AI providers have been removed.","provider":"groq"}),400

    supplied_key=(d.get("api_key") or "").strip()
    old_key=runtime_cfg.get("groq_key")
    if supplied_key:
        runtime_cfg["groq_key"]=supplied_key
    elif not old_key:
        return jsonify({"error":"Enter your Groq API key first."}),400

    ok,status,_resp=_groq_probe()
    if not ok:
        if supplied_key:
            if old_key:
                runtime_cfg["groq_key"]=old_key
            else:
                runtime_cfg.pop("groq_key",None)
        runtime_cfg["groq_provider_status"]=status
        runtime_cfg["ai_provider"]="groq"
        runtime_cfg["last_ai_provider"]="groq"
        save_config(runtime_cfg)
        message={
            "invalid_key":"Groq rejected the API key.",
            "quota":"Groq accepted the key format but the current quota/rate limit is unavailable.",
            "network":"Could not reach Groq from the Render server.",
            "missing":"No Groq API key is configured."
        }.get(status,f"Groq check failed ({status}).")
        return jsonify({"ok":False,"error":message,"provider":"groq","status":status,"selected":"groq","active":"groq"}),502

    runtime_cfg["groq_provider_status"]="ok"
    runtime_cfg["groq_provider_verified_at"]=time.time()
    runtime_cfg["ai_provider"]="groq"
    runtime_cfg["last_ai_provider"]="groq"
    save_config(runtime_cfg)
...
loadVoiceConfig();
async function loadAiProviders(verify=false){
  try{
    const r=await fetch('/api/ai/providers'+(verify?'?verify=1':''));
    const d=await r.json();
    const p=(d.providers||[])[0]||{};
    $('aiProviderInfo').innerHTML='<b>Provider:</b> Groq<br><b>Model:</b> '+(p.model||'—')+'<br><b>Status:</b> '+(p.verified?'Working':(p.status||'Not verified'))+(p.configured?'':'<br><b>Key:</b> Not configured');
  }catch(e){$('aiProviderInfo').textContent='Could not reach the Jarvis server.'}
}
$('saveAiProvider').onclick=async()=>{
  const key=$('aiProviderKey').value.trim();
  try{
    if(!key){$('aiProviderInfo').textContent='Enter a Groq API key first.';return}
    $('saveAiProvider').disabled=true;$('aiProviderInfo').textContent='Verifying Groq…';
    const r=await fetch('/api/ai/providers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider:'groq',api_key:key})});
    const d=await r.json();
    if(!r.ok)throw Error(d.error||'Groq verification failed.');
    $('aiProviderKey').value='';
    await loadAiProviders();
  }catch(e){$('aiProviderInfo').textContent=e.message||'Could not verify Groq.'}
  finally{$('saveAiProvider').disabled=false}
};
$('aiProviderStatus').onclick=()=>loadAiProviders(true);loadAiProviders();
