...
def _firebase_access_token():
    ...
    payload = _b64url(json.dumps({"iss":info["client_email"],"scope":"https://www.googleapis.com/auth/datastore","aud":"https://oauth2.googleapis.com/token","iat":now,"exp":now+3600},separators=(",",":")).encode())
...
def load_chat():
    ...
    if isinstance(remote,list): return remote[-100:]
...
def save_chat(items):
    ...
    if _firebase_service_account():
        if not _firebase_memory_save(trimmed):
            try:
...
def handle_command(text):
    ...
    if lower in ("clear chat","reset","clear conversation"):
        conversation_history[:]=[conversation_history[0]];save_chat([]);return "Cleared - what would you like to talk about?",None
    ...
    if topic_match:
        ...
        conversation_history.append({"role":"user","content":f"Investigate this topic: {topic}. Web context: {web_context}"})
        reply=ask_groq(conversation_history);conversation_history.append({"role":"assistant","content":reply})
        return reply,None
    conversation_history.append({"role":"user","content":stripped});reply=ask_groq(conversation_history);conversation_history.append({"role":"assistant","content":reply});save_chat([m for m in conversation_history[1:] if m.get("role") in ("user","assistant")])
...
def api_photo():
    ...
    conversation_history.append({"role":"user","content":question or "I uploaded a photo for you to inspect."})
