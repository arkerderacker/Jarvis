...
def _strip_html(text):
 text=re.sub(r"(?is)<script[^>]*>.*?</script>|<style[^>]*>.*?<style>.quiz-card{padding:16px;margin-top:8px;min-width:min(420px,82vw)}.quiz-title{font-size:16px;font-weight:700}.quiz-progress{font-size:10px;opacity:.65;margin:6px 0 14px}.quiz-question{font-size:14px;line-height:1.5;margin-bottom:12px}.quiz-options{display:grid;gap:8px}.quiz-option{text-align:left;padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:inherit;cursor:pointer}.quiz-option.correct{border-color:#43e58c;background:rgba(67,229,140,.14)}.quiz-option.wrong{border-color:#ff6262;background:rgba(255,98,98,.12)}.quiz-feedback{font-size:11px;min-height:30px;margin-top:10px}.quiz-nav{display:flex;justify-content:space-between;margin-top:8px}.image-gen-card{padding:12px;margin-top:8px}.image-gen-loading{display:flex;align-items:center;gap:10px;font-size:12px}.loader-ring{width:20px;height:20px;border:2px solid rgba(255,255,255,.18);border-top-color:var(--accent);border-radius:50%;animation:jarvisSpin .8s linear infinite}.generated-image{display:block;max-width:min(560px,82vw);max-height:55vh;border-radius:16px;margin-top:8px}.code-card{padding:12px;margin-top:8px}.code-card pre{white-space:pre-wrap;overflow:auto;max-height:45vh;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}@keyframes jarvisSpin{to{transform:rotate(360deg)}}</style></style>|<noscript[^>]*>.*?</noscript>"," ",text or "");text=re.sub(r"(?s)<[^>]+>"," ",text);return re.sub(r"\s+"," ",html.unescape(text)).strip()
...
def _quiz_from_ai(topic):
    prompt = ('Create a 5-question multiple-choice quiz about '+topic+'. Return ONLY valid JSON, no markdown and no prose. '
              'Schema: {"title": string, "questions": [{"question": string, "options": [string,string,string,string], "answer": integer, "explanation": string}]}. '
              'The answer is the zero-based index of exactly one correct option. Keep questions suitable for the topic and unambiguous.')
    raw=ask_ai([conversation_history[0],{"role":"user","content":prompt}])
    candidate=raw.strip()
    ...
    if not isinstance(qs,list):return None
    clean=[]
    for q in qs[:10]:
        ...
        if not isinstance(options,list) or len(options)<2 or ans<0 or ans>=len(options):continue
        clean.append({'question':str(q.get('question') or '').strip(),'options':[str(x) for x in options[:4]],'answer':ans,'explanation':str(q.get('explanation') or '').strip()})
    return {'title':str(data.get('title') or (topic+' Quiz')),'questions':clean} if clean else None
...
def handle_command(text):
    ...
    if re.search(r"\b(create|make|build|write|code)\b.*\b(html|css|javascript|python|typescript|java|c\+\+?|rust|go|sql)\b",lower):
        language='HTML' if 'html' in lower else ('CSS' if 'css' in lower else ('JavaScript' if 'javascript' in lower or re.search(r'\bjs\b',lower) else 'Python'))
...
def api_image_search():
    ...
    try:
        r=requests.get("https://www.bing.com/images/search",params={"q":q,"form":"HDRSC2"},headers={"User-Agent":"Mozilla/5.0"},timeout=12)
        if not r.ok:return jsonify({"images":[]})
        found=[]
        for m in re.finditer(r'"murl":"(.*?)"',r.text):
            try:u=json.loads('"'+m.group(1)+'"')
            except Exception:u=html.unescape(m.group(1))
            if u.startswith("http") and u not in [x["url"] for x in found]:found.append({"url":u,"title":q})
            if len(found)>=8:break
        return jsonify({"images":found})
    except Exception as e:return jsonify({"images":[],"error":str(e)})
