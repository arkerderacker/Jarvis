...
def api_photos():
    ...
    if items is not None:
        ...
        for item in sorted(items,key=lambda x:x.get("updated",""),reverse=True):
            ...
            photos.append({"name":name,"url":"/api/photos/file/"+requests.utils.quote(name,safe="")})
        return jsonify({"photos":photos,"persistent":True})
    photos=[]
...
def api_photo_upload():
    ...
    if _firebase_service_account():
        if not _firebase_storage_upload(data,object_name,content_type):
            return jsonify({"error":"Firebase Storage is configured but the photo could not be saved. Check FIREBASE_STORAGE_BUCKET and that Cloud Storage is enabled for this Firebase project."}),503
        return jsonify({"ok":True,"name":name,"url":"/api/photos/file/"+requests.utils.quote(name,safe=""),"persistent":True})
    try:
...
def api_photo_delete():
    ...
    if remote_items is not None:
        ...
        return jsonify({"ok":ok,"persistent":True}) if ok else (jsonify({"error":"Photo could not be deleted from Firebase Storage."}),502)
    path=os.path.join(PHOTOS_DIR,name)
...
def api_usage():
    ...
    memory_used=_firebase_memory_usage_estimate()
    firebase={
        "configured": bool(_firebase_service_account()),
        "storage_used_bytes": storage_used,
        "storage_free_bytes": (5*1024**3-storage_used) if storage_used is not None else None,
        "storage_free_tier_bytes": 5*1024**3,
        "firestore_memory_estimate_bytes": memory_used,
        "firestore_free_tier_bytes": 1024**3
    }
