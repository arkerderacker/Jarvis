...
def handle_command(text):
    ...
    h.append({"role":"system","content":f"Live clock context: Singapore time is {now.strftime('%A, %d %B %Y %H:%M:%S')} (Asia/Singapore). Treat this as the actual current date and time when relevant; do not guess."})
...
def api_time():
    now=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))
    return jsonify({"iso":now.isoformat(),"date":now.strftime("%A, %d %B %Y"),"time":now.strftime("%H:%M:%S"),"timezone":"Asia/Singapore"})
...
#jarvisBootFX{position:fixed;inset:0;z-index:90;display:grid;place-items:center;background:radial-gradient(circle at 50% 45%,color-mix(in srgb,var(--accent) 14%,#03070b),#03070b 68%);transition:opacity .7s ease,visibility .7s ease}
...
#jarvisTip{position:fixed;left:50%;bottom:112px;transform:translateX(-50%);padding:10px 14px;border-radius:14px;z-index:25;opacity:0;transition:.3s;pointer-events:none;background:rgba(12,18,27,.76);backdrop-filter:blur(18px);border:1px solid rgba(255,255,255,.12);font-size:11px}
#jarvisTip.show{opacity:1}
#jarvisImmersive{position:fixed;inset:0;background:#03070b;z-index:5000;display:none;opacity:0;transition:opacity .42s ease,transform .42s ease;transform:scale(1.035);isolation:isolate} 
#jarvisImmersive.open{display:block;opacity:1;transform:scale(1)}
#jarvisImmersive video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:.72;transform:scaleX(-1);z-index:0;filter:saturate(.9) contrast(1.04)}
#jarvisDraw{position:absolute;inset:0;width:100%;height:100%;touch-action:none;z-index:2;pointer-events:none}
.jiHud{position:absolute;z-index:3;inset:0;border:1px solid color-mix(in srgb,var(--accent) 30%,transparent);box-shadow:inset 0 0 100px color-mix(in srgb,var(--accent) 10%,transparent);pointer-events:none}
.jiTop{position:absolute;z-index:4;top:18px;left:22px;right:22px;display:flex;justify-content:space-between;font-size:10px;letter-spacing:.15em}
.jiMenu{position:absolute;z-index:5;bottom:22px;left:50%;transform:translateX(-50%);display:flex;gap:7px;align-items:center;padding:9px 11px;border-radius:16px;background:rgba(4,8,12,.82);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,.12)}
...
$('camToggle').onclick=async()=>{$('handcam').classList.toggle('hidden');camVisible=!$('handcam').classList.contains('hidden');$('camToggle').textContent=camVisible?'Hide':'Show';if(camVisible&&!handsReady){try{await initHands();$('status').textContent='ONLINE'}catch(e){showTip('Camera access failed: '+(e.message||'permission denied'))}}saveRemoteSettings()};$('resetView').onclick=()=>{targetZ=34;wordSprites.forEach(s=>s.material.opacity=1)};$('clearWords').onclick=()=>rebuildWords('web photos notes schedule');
...
let handsReady=false, handsStream=null, handsLoopRunning=false, handsFrameBusy=false;
async function initHands(){
 ...
 const v=$('handcam');
 if(!v) throw new Error('Hand camera element is missing.');
 ...
 handsStream=stream;v.srcObject=stream;v.muted=true;v.playsInline=true;
 await v.play();
 const hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});
 hands.setOptions({maxNumHands:2,modelComplexity:1,minDetectionConfidence:.58,minTrackingConfidence:.58});
 hands.onResults(res=>{
   const list=res.multiHandLandmarks||[];
   if(!list.length){handSeen=false;$('pointer').style.display='none';window.jarvisHandleHands?.([]);return}
   handSeen=true;
   const primary=list[0],x=1-primary[8].x,y=primary[8].y;
   $('pointer').style.display='block';$('pointer').style.left=(x*100)+'%';$('pointer').style.top=(y*100)+'%';
   window.jarvisHandleHands?.(list);
   if(!immersive){
     let span=Math.hypot(primary[0].x-primary[12].x,primary[0].y-primary[12].y);
     targetZ=THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(span,.18,.42,48,13),13,48);
     jarvis?.position.set(0,0,0);
     const pinch=Math.hypot(primary[8].x-primary[4].x,primary[8].y-primary[4].y)<.055;
     if(pinch&&!pinchLatch){pinchLatch=true;let hit=hitAt(x,y);if(hit)selectWord(hit)}
     if(!pinch)pinchLatch=false;
   }
 });
 handsReady=true;handsLoopRunning=true;
 const tick=async()=>{
   if(!handsLoopRunning)return;
   if(!handsFrameBusy && v.readyState>=2){
     handsFrameBusy=true;
     try{await hands.send({image:v})}catch(e){console.warn('Hand tracking frame failed:',e)}finally{handsFrameBusy=false}
   }
...
async function setup(){
 try{
   ...
   if(d.configured){
     ...
     window.jarvisActivity?.('CORE','Saved Groq configuration detected. Starting Jarvis.','groq');
     try{await initHands();$('status').textContent='ONLINE';}catch(e){$('status').textContent='CAMERA NEEDS PERMISSION';$('err') && ($('err').textContent='Camera access was not granted automatically. Use your browser camera permission or refresh and initialize again.');}
     window.runJarvisBoot?.();
     addMsg('j','Hello. Jarvis is online. Speak with the microphone, type below, or use hand gestures.');
   }
 }catch(e){$('status').textContent='SERVER OFFLINE';}
}
async function initializeJarvis(){
 const key=$('key').value.trim();
 if(!key){$('err').textContent='A Groq API key is required.';return}
 const btn=$('begin');btn.disabled=true;$('err').textContent='';$('status').textContent='CONNECTING';
 try{
   const r=await fetch('/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({groq_key:key,save:$('save').checked}),cache:'no-store'});
   const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'The server rejected the Groq API key.');
   $('status').textContent='CAMERA PERMISSION';
   try{await initHands();}catch(e){
     $('status').textContent='CAMERA UNAVAILABLE';
     $('err').textContent='Groq is connected, but camera access failed: '+(e.message||'permission denied')+'. Jarvis can still open, and you can retry camera access from Settings.';
   }
   $('setup').classList.add('hidden');
   window.jarvisActivity?.('CORE','Groq connected. Jarvis initialized.','groq');
   addMsg('j',handsReady?'Jarvis initialized. Camera and gesture control are ready.':'Jarvis initialized. Camera is unavailable, but chat and voice remain available.');
   window.runJarvisBoot?.();
 }catch(e){
   $('err').textContent=e.message||'Could not reach the server.';$('status').textContent='INITIALIZATION FAILED';btn.disabled=false;
   document.getElementById('jarvisVisualCore')?.classList.add('error');
 }
}
$('begin').onclick=initializeJarvis;
...
<div id="jarvisImmersive" aria-hidden="true">
 <video id="jarvisImmersiveVideo" autoplay muted playsinline></video><canvas id="jarvisDraw"></canvas><div class="jiHud"></div>
 ...
 <div class="jiMenu">
   ...
   <input id="jarvisColor" class="jiSwatch" type="color" value="#46e0d0" title="Drawing colour">
...
<script>
/* Jarvis interaction layer: immersive camera workspace, gestures, state/activity UI, and quiet proactive assistance. */
(()=>{
 const $=id=>document.getElementById(id), root=document.documentElement;
 const core=$('jarvisVisualCore'), boot=$('jarvisBootFX'), wave=$('jarvisWave'), imm=$('jarvisImmersive');
 let immersive=false,lastModeToggle=0,fistSince=0,drawColor=localStorage.getItem('jarvisDrawColor')||'#46e0d0';
 let strokes=[],currentStroke=null,selectedStroke=null,pinchMove=false,lastDraw=null,drawing=false;
 let panelDrag=null,twoHandState=null,lastProactive=0;
 const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
 const colors=['#46e0d0','#59a7ff','#a97cff','#ffb84d','#ff5f7a','#67e48b'];
 function safeText(v){return String(v??'').replace(/[<>&]/g,m=>({'<':'&lt;','>':'&gt;','&':'&amp;'}[m]))}
 function makeWave(){if(!wave)return;wave.innerHTML='';for(let i=0;i<28;i++){const b=document.createElement('i');b.dataset.i=i;wave.appendChild(b)}}makeWave();
 function setState(state){
   if(!core)return;core.className=state;
   const status=$('status');if(status&&state!=='error')status.textContent=state.toUpperCase();
   document.querySelectorAll('#jarvisTools .jt').forEach(x=>x.classList.remove('active'));
   const map={listening:'mic',thinking:'groq',speaking:'mic'};if(map[state])document.querySelector(`#jarvisTools .jt[data-tool="${map[state]}"]`)?.classList.add('active');
 }
 function activity(label,text,tool){
   const box=$('jarvisActivityItems');if(!box)return;
   if(tool)document.querySelectorAll('#jarvisTools .jt').forEach(x=>x.classList.toggle('active',x.dataset.tool===tool));
   const el=document.createElement('div');el.className='jaItem';el.innerHTML='<b>'+safeText(label)+'</b>'+safeText(text);box.prepend(el);while(box.children.length>5)box.lastElementChild.remove();
 }
 function showTip(text){const t=$('jarvisTip');if(!t)return;t.textContent='JARVIS · '+text;t.classList.add('show');clearTimeout(t._tm);t._tm=setTimeout(()=>t.classList.remove('show'),5000)}
 window.jarvisActivity=activity;window.jarvisSetState=setState;window.jarvisShowTip=showTip;
 if($('jarvisColor')){$('jarvisColor').value=drawColor;$('jarvisColor').onchange=e=>{drawColor=e.target.value;localStorage.setItem('jarvisDrawColor',drawColor)}}
 function runJarvisBoot(){
   if(!boot)return;
   boot.classList.remove('done');boot.classList.remove('replay');void boot.offsetWidth;boot.classList.add('replay');
   const meta=boot.querySelector('.jbMeta');if(meta)meta.textContent='JARVIS CORE · ONLINE';
   setState('thinking');
   setTimeout(()=>{boot.classList.add('done');setState('idle');$('status').textContent=handsReady?'ONLINE':'CAMERA UNAVAILABLE'},1400);
 }
 window.runJarvisBoot=runJarvisBoot;
 function canvasPointXY(x,y){const c=$('jarvisDraw'),r=c.getBoundingClientRect();return{x:clamp((x-r.left)/r.width,0,1),y:clamp((y-r.top)/r.height,0,1)}}
 function redraw(){
   const c=$('jarvisDraw'),ctx=c?.getContext('2d');if(!c||!ctx)return;ctx.clearRect(0,0,c.width,c.height);
   for(const st of strokes){if(!st?.points?.length)continue;ctx.strokeStyle=st.color||drawColor;ctx.lineWidth=(st.size||4)*Math.min(devicePixelRatio||1,2);ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();st.points.forEach((p,i)=>{const x=p.x*c.width,y=p.y*c.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});if(st.points.length===1)ctx.lineTo(st.points[0].x*c.width+.01,st.points[0].y*c.height+.01);ctx.stroke()}
 }
 function resizeCanvas(){const c=$('jarvisDraw');if(!c)return;const d=Math.min(devicePixelRatio||1,2);c.width=Math.floor(innerWidth*d);c.height=Math.floor(innerHeight*d);c.style.width=innerWidth+'px';c.style.height=innerHeight+'px';redraw()}
 addEventListener('resize',()=>{if(immersive)resizeCanvas()});
 function enterImmersive(){
   if(immersive||performance.now()-lastModeToggle<1200)return;
   if(!handsReady){showTip('Camera and hand tracking are not ready yet.');return}
   immersive=true;lastModeToggle=performance.now();fistSince=0;panelDrag=null;selectedStroke=null;pinchMove=false;
   imm.classList.add('open');imm.setAttribute('aria-hidden','false');
   const src=$('handcam'),iv=$('jarvisImmersiveVideo');if(src?.srcObject){iv.srcObject=src.srcObject;iv.play().catch(()=>{})}
   resizeCanvas();activity('IMMERSIVE','Camera workspace active.','vision');setState('thinking');showTip('Immersive mode · point to draw · pinch to move.');
   setTimeout(()=>setState('listening'),650);
 }
 function exitImmersive(){
   if(!immersive||performance.now()-lastModeToggle<900)return;
   immersive=false;lastModeToggle=performance.now();fistSince=0;drawing=false;currentStroke=null;selectedStroke=null;pinchMove=false;lastDraw=null;twoHandState=null;
   imm.classList.remove('open');imm.setAttribute('aria-hidden','true');activity('IMMERSIVE','Returned to the main Jarvis interface.','vision');setState('idle');showTip('Immersive mode closed.');
 }
 window.jarvisEnterImmersive=enterImmersive;window.jarvisExitImmersive=exitImmersive;$('jarvisExit').onclick=exitImmersive;
 $('jarvisClear').onclick=()=>{strokes=[];selectedStroke=null;redraw();activity('DRAW','Canvas cleared.','vision')};
 $('jarvisUndo').onclick=()=>{strokes.pop();selectedStroke=null;redraw();activity('DRAW','Last stroke removed.','vision')};
 async function saveCreation(name){
   if(!strokes.length){showTip('There is nothing to save yet.');return}
   name=(name||prompt('What would you like to name your creation?')||'').trim();if(!name)return;
   const r=await fetch('/api/creations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,strokes})});const d=await r.json();if(!r.ok)throw Error(d.error||'Could not save creation.');
   activity('MEMORY','Saved “'+name+'”.','memory');showTip('Saved creation: '+name);
 }
 async function loadCreation(name){
   name=(name||prompt('Which creation should I load?')||'').trim();if(!name)return;
   const r=await fetch('/api/creations');const d=await r.json();const found=(d.creations||[]).find(x=>String(x.name||'').toLowerCase()===name.toLowerCase());
   if(!found){showTip('I could not find “'+name+'”.');return}strokes=Array.isArray(found.strokes)?found.strokes:[];redraw();activity('MEMORY','Loaded “'+found.name+'”.','memory');showTip('Loaded creation: '+found.name);
 }
 $('jarvisSave').onclick=()=>saveCreation().catch(e=>showTip(e.message));$('jarvisLoad').onclick=()=>loadCreation().catch(e=>showTip(e.message));
 const canvas=$('jarvisDraw');
 canvas.addEventListener('pointerdown',e=>{if(!immersive)return;const p=canvasPointXY(e.clientX,e.clientY);drawing=true;currentStroke={color:drawColor,size:4,points:[p]};strokes.push(currentStroke);canvas.setPointerCapture?.(e.pointerId);redraw()});
 canvas.addEventListener('pointermove',e=>{if(!immersive||!drawing||!currentStroke)return;currentStroke.points.push(canvasPointXY(e.clientX,e.clientY));redraw()});
 canvas.addEventListener('pointerup',()=>{drawing=false;currentStroke=null});canvas.addEventListener('pointercancel',()=>{drawing=false;currentStroke=null});
 function strokeHit(p){let best=null,bestD=.045;for(const st of strokes){for(const q of st.points){const d=Math.hypot(q.x-p.x,q.y-p.y);if(d<bestD){bestD=d;best=st;break}}}return best}
 function isFingerExtended(h,tip,pip){return h[tip].y<h[pip].y-0.025}
 function isFist(h){const wrist=h[0];const midDist=Math.hypot(h[12].x-wrist.x,h[12].y-wrist.y);return [8,12,16,20].every(t=>Math.hypot(h[t].x-wrist.x,h[t].y-wrist.y)<midDist*.82)}
 function handleNormalGesture(h,ix,iy){
   const together=Math.hypot(h[8].x-h[12].x,h[8].y-h[12].y)<.07;
   if(together&&!panelDrag){
     const candidates=[...document.querySelectorAll('#settings,#aboutPanel,#cameraPanel,#developerPanel,#usagePanel,#musicPanel,#library')].filter(x=>x&&(x.classList.contains('open')||getComputedStyle(x).display!=='none'));
     const target=candidates.find(x=>x.getBoundingClientRect().contains(innerWidth*ix,innerHeight*iy));
     if(target)panelDrag={el:target,x:ix,y:iy,left:parseFloat(target.dataset.jx)||0,top:parseFloat(target.dataset.jy)||0};
   }else if(!together&&panelDrag){panelDrag=null}
   if(panelDrag){const dx=(ix-panelDrag.x)*innerWidth,dy=(iy-panelDrag.y)*innerHeight;panelDrag.left+=dx;panelDrag.top+=dy;panelDrag.x=ix;panelDrag.y=iy;panelDrag.el.style.transform=`translate(${panelDrag.left}px,${panelDrag.top}px)`;panelDrag.el.classList.add('jiDragging')}
   else document.querySelectorAll('.jiDragging').forEach(x=>x.classList.remove('jiDragging'));
 }
 function handleImmersiveHands(hands){
   const primary=hands[0];if(!primary)return;
   const idx=primary[8],thumb=primary[4],ix=1-idx.x,iy=idx.y;
   const fist=isFist(primary);
   if(fist){
     if(!fistSince)fistSince=performance.now();
     drawing=false;currentStroke=null;
     if(performance.now()-fistSince>=5000){exitImmersive();return}
   }else fistSince=0;
   const pinch=Math.hypot(idx.x-thumb.x,idx.y-thumb.y)<.065;
   const p={x:clamp(ix,0,1),y:clamp(iy,0,1)};
   if(pinch){
     if(!selectedStroke){const hit=strokeHit(p);if(hit){selectedStroke=hit;activity('DRAW','Grabbed a creation.','vision')}}
     if(selectedStroke){if(!lastDraw)lastDraw=p;const dx=p.x-lastDraw.x,dy=p.y-lastDraw.y;selectedStroke.points.forEach(q=>{q.x=clamp(q.x+dx,0,1);q.y=clamp(q.y+dy,0,1)});lastDraw=p;pinchMove=true;redraw()}
     return;
   }
   if(pinchMove){pinchMove=false;selectedStroke=null;lastDraw=null}
   if(!fist && isFingerExtended(primary,8,6)){
     if(!drawing){currentStroke={color:drawColor,size:4,points:[p]};strokes.push(currentStroke);drawing=true}
     else currentStroke.points.push(p);
     redraw();
   }else{drawing=false;currentStroke=null}
   if($('pointer')){$('pointer').style.left=(ix*100)+'%';$('pointer').style.top=(iy*100)+'%'}
   if(hands.length>=2){
     const a=hands[0][8],b=hands[1][8],center={x:(a.x+b.x)/2,y:(a.y+b.y)/2},dist=Math.max(.08,Math.hypot(a.x-b.x,a.y-b.y));
     if(twoHandState){const ratio=clamp(dist/twoHandState.dist,.82,1.22);const scale=clamp(ratio,.85,1.18);const cx=twoHandState.cx,cy=twoHandState.cy;for(const st of strokes)for(const q of st.points){q.x=clamp(cx+(q.x-cx)*scale,0,1);q.y=clamp(cy+(q.y-cy)*scale,0,1)}twoHandState={dist,cx,cy};redraw()}
     else twoHandState={dist,cx:center.x,cy:center.y};
   }else twoHandState=null;
 }
 window.jarvisHandleHands=function(hands){
   if(!Array.isArray(hands)||!hands.length){handSeen=false;if(!immersive)fistSince=0;return}
   handSeen=true;const h=hands[0],ix=1-h[8].x,iy=h[8].y;
   if(!immersive){
     if(isFist(h)){if(!fistSince)fistSince=performance.now();if(performance.now()-fistSince>=5000){enterImmersive();return}}else fistSince=0;
     handleNormalGesture(h,ix,iy);return;
   }
   handleImmersiveHands(hands);
 };
 window.jarvisVisionRequest=async function(question){
   if(!immersive)return false;const v=$('jarvisImmersiveVideo');if(!v||!v.videoWidth)throw Error('Immersive camera is not ready.');
   const c=document.createElement('canvas'),w=Math.min(v.videoWidth,1280),h=Math.min(v.videoHeight,Math.round(w*v.videoHeight/v.videoWidth));c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.drawImage(v,0,0,w,h);
   if(/what did i draw|what have i drawn|describe my drawing|what is my drawing/i.test(question||'')){
     const dc=$('jarvisDraw');if(dc){ctx.globalAlpha=.92;ctx.drawImage(dc,0,0,w,h);ctx.globalAlpha=1}}
   const image=c.toDataURL('image/jpeg',.78);$('jarvisVisionStatus').textContent='VISION ANALYZING';activity('VISION','Analyzing the current frame only.','vision');setState('thinking');
   try{const r=await fetch('/api/photo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image,question:question||'What am I holding? Describe what you see in the current frame.'})});const d=await r.json();if(!r.ok)throw Error(d.error||'Vision failed.');$('jarvisVisionStatus').textContent='VISION READY';setState('listening');return d.reply}catch(e){$('jarvisVisionStatus').textContent='VISION ERROR';setState('error');throw e}
 };
 // Never auto-hide the boot overlay before initialization. It is replayed after successful setup.
 setState('idle');
 // Quiet proactive assistance: only reminders or explicit scheduled events may speak without a user request.
 window.jarvisMaybeTip=function(context){return;};
 // Existing UI panels remain draggable with index+middle together, but immersive mode owns the gestures while open.
 document.querySelectorAll('#settings,#aboutPanel,#cameraPanel,#developerPanel,#usagePanel,#musicPanel,#library').forEach(el=>{el.dataset.jx='0';el.dataset.jy='0'});
 setInterval(()=>{
   if(typeof listening!=='undefined'&&listening)setState('listening');else if(typeof isJarvisSpeaking!=='undefined'&&isJarvisSpeaking)setState('speaking');else if(typeof status!=='undefined'&&status.textContent==='THINKING')setState('thinking');else if(!immersive)setState('idle');
   const level=typeof voiceLevel!=='undefined'?voiceLevel:0;document.querySelectorAll('#jarvisWave i').forEach((b,i)=>{const dist=Math.abs(i-14)/14,n=Math.max(.12,(1-dist)*level*(.75+Math.random()*.7)+.12);b.style.height=(n*100)+'%';b.style.transform=`scaleY(${.65+level*1.4})`});
 },70);
 // On-demand immersive vision interception. Normal chat remains untouched for every other question.
 const oldSend=window.send;
 if(typeof oldSend==='function'&&!window.__jarvisSendWrapped){window.__jarvisSendWrapped=true;window.send=async function(){const q=($('text')?.value||'').trim();if(immersive&&/what am i holding|what is this|what's this|what do you see|describe (this|what you see)|identify (this|what)|what did i draw|what have i drawn|describe my drawing|what is my drawing/i.test(q)){if($('text'))$('text').value='';addMsg('user',q);try{const reply=await window.jarvisVisionRequest(q);addMsg('j',reply);await speak(reply);rebuildWords(q+' '+reply)}catch(e){addMsg('j',e.message||'I could not analyze the current camera frame.')}return}return oldSend.apply(this,arguments)}}
})();
</script>
</body></html>'''
