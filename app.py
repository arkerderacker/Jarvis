...
CREATIONS_FILE = os.path.join(JARVIS_DIR, "creations.json")
...
def ask_ai(history):
    ...
    try:
        ...
        live=singapore_now()
        live_context=(f"\nLIVE SYSTEM CLOCK (authoritative): Singapore time is {live.strftime('%A, %d %B %Y, %I:%M:%S %p')} (Asia/Singapore). "
                      "Use this value for time/date reasoning; do not guess the current time.\n")
        messages=[dict(m) for m in history]
        if messages and messages[0].get("role")=="system":
            messages[0]["content"]=str(messages[0].get("content",""))+live_context
        else:
...
def tell_time():
    n=singapore_now(); return f"It's {n.strftime('%I:%M %p').lstrip('0')} on {n.strftime('%A, %B %d, %Y')}."
def tell_date():
    n=singapore_now(); return f"Today is {n.strftime('%A, %B %d, %Y')}."
def write_note(text):
    text=text.strip()
...
def singapore_now():
    try:
        return datetime.datetime.now(ZoneInfo("Asia/Singapore")) if ZoneInfo else datetime.datetime.utcnow()+datetime.timedelta(hours=8)
    except Exception:
        return datetime.datetime.utcnow()+datetime.timedelta(hours=8)

def load_creations():
    try:
        if not os.path.exists(CREATIONS_FILE): return {}
...
def save_creations(data):
    tmp=CREATIONS_FILE+".tmp"
    with open(tmp,"w",encoding="utf-8") as f: json.dump(data,f,ensure_ascii=False,indent=2)
    os.replace(tmp,CREATIONS_FILE)

@app.route("/api/time")
def api_time():
    n=singapore_now()
    return jsonify({"ok":True,"timezone":"Asia/Singapore","iso":n.isoformat(),"date":n.strftime("%A, %d %B %Y"),"time":n.strftime("%I:%M:%S %p").lstrip("0")})

@app.route("/api/creations",methods=["GET","POST","DELETE"])
def api_creations():
    data=load_creations()
    if request.method=="GET":
        return jsonify({"ok":True,"creations":[{"name":k,"updated":v.get("updated","")} for k,v in sorted(data.items(),key=lambda x:x[1].get("updated",""),reverse=True)]})
    body=request.get_json(force=True,silent=True) or {}
    name=(body.get("name") or "").strip()
    if not name or len(name)>100: return jsonify({"error":"A creation name is required (1-100 characters)."}),400
    if request.method=="DELETE":
        data.pop(name,None); save_creations(data); return jsonify({"ok":True})
    objects=body.get("objects")
    if not isinstance(objects,list) or len(objects)>5000: return jsonify({"error":"Invalid creation data."}),400
    # Keep only drawing data; never accept arbitrary server paths/code.
    clean=[]
    for obj in objects:
        if not isinstance(obj,dict): continue
        pts=obj.get("points")
        if isinstance(pts,list) and len(pts)<=5000:
            clean_pts=[]
            for pt in pts:
                if isinstance(pt,list) and len(pt)==2:
                    try: clean_pts.append([max(0,min(1,float(pt[0]))),max(0,min(1,float(pt[1])))])
                    except (TypeError,ValueError): pass
            if clean_pts: clean.append({"points":clean_pts,"width":max(.5,min(30,float(obj.get("width",5)))),"color":str(obj.get("color","#46e0d0"))[:20]})
    data[name]={"updated":datetime.datetime.now(datetime.timezone.utc).isoformat(),"objects":clean}
    save_creations(data)
    return jsonify({"ok":True,"name":name,"updated":data[name]["updated"]})

@app.route("/api/creations/<path:name>")
def api_creation(name):
    data=load_creations()
    item=data.get(name)
    if not item: return jsonify({"error":"Creation not found."}),404
    return jsonify({"ok":True,"name":name,"updated":item.get("updated",""),"objects":item.get("objects",[])})
...
@app.route("/api/live-vision",methods=["POST"])
def api_live_vision():
    if not is_configured(): return jsonify({"error":"Not configured yet."}),400
    data=request.get_json(force=True,silent=True) or {}
    image=data.get("image") or ""; question=(data.get("question") or "What am I holding?").strip()
    if not image.startswith("data:image/"): return jsonify({"error":"Invalid live camera frame."}),400
    if len(image)>3_500_000: return jsonify({"error":"Live frame is too large."}),413
    return jsonify({"ok":True,"reply":ask_groq_photo(image,question),"saved":False})
...
/* Futuristic command-center layer */
#coreHud{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);width:min(42vw,430px);aspect-ratio:1;z-index:8;pointer-events:none;transition:transform .5s ease,opacity .35s ease,filter .35s ease}
.core-ring{position:absolute;inset:8%;border:1px solid color-mix(in srgb,var(--accent) 55%,transparent);border-radius:50%;box-shadow:0 0 34px color-mix(in srgb,var(--accent) 22%,transparent),inset 0 0 34px color-mix(in srgb,var(--accent) 10%,transparent);animation:coreSpin 18s linear infinite}
.core-ring.r2{inset:17%;border-style:dashed;animation-duration:11s;animation-direction:reverse}
.core-ring.r3{inset:27%;border-color:color-mix(in srgb,var(--accent) 35%,transparent);animation-duration:7s}
.core-energy{position:absolute;inset:35%;border-radius:50%;background:radial-gradient(circle,#fff 0%,var(--accent) 12%,color-mix(in srgb,var(--accent) 38%,transparent) 38%,transparent 70%);filter:drop-shadow(0 0 28px var(--accent));animation:coreBreath 2.8s ease-in-out infinite}
.core-wave{position:absolute;inset:5%;border-radius:50%;border:2px solid transparent;box-shadow:0 0 0 1px color-mix(in srgb,var(--accent) 18%,transparent);transform:scale(calc(1 + var(--voice,0)*.08));transition:transform .08s linear}
#coreHud.listening .core-ring{animation-duration:4s;box-shadow:0 0 50px color-mix(in srgb,var(--accent) 34%,transparent)}
#coreHud.thinking .core-ring{animation-duration:2.2s}
#coreHud.thinking .core-energy{background:radial-gradient(circle,#fff4c7 0%,#f3b53d 16%,rgba(243,181,61,.32) 40%,transparent 70%);filter:drop-shadow(0 0 32px #f3b53d)}
#coreHud.speaking .core-wave{animation:wavePulse .18s ease-in-out infinite alternate}
#coreHud.error .core-energy{background:radial-gradient(circle,#fff 0%,#ff4d5e 14%,rgba(255,77,94,.35) 42%,transparent 70%);filter:drop-shadow(0 0 32px #ff4d5e)}
@keyframes coreSpin{to{transform:rotate(360deg)}}@keyframes coreBreath{50%{transform:scale(1.08);opacity:.82}}@keyframes wavePulse{to{transform:scale(1.08)}}
#activityPanel{position:fixed;right:18px;top:70px;width:min(300px,calc(100vw - 36px));z-index:45;max-height:38vh;overflow:auto;border-radius:16px;padding:12px;opacity:0;transform:translateY(-8px);pointer-events:none;transition:.25s}
#activityPanel.show{opacity:1;transform:translateY(0);pointer-events:auto}
.activity-item{padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.07);font-size:11px;color:var(--muted)}.activity-item b{display:block;color:#fff;font-size:10px;letter-spacing:.1em;margin-bottom:3px}.tool-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--accent);box-shadow:0 0 10px var(--accent);margin-right:6px}
#toolBar{position:fixed;left:50%;bottom:78px;transform:translateX(-50%);z-index:15;display:flex;gap:7px;padding:7px 9px;border-radius:999px;background:rgba(5,9,14,.55);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,.1);font-size:9px;letter-spacing:.09em;color:rgba(255,255,255,.35);pointer-events:none}
.tool{transition:.2s}.tool.active{color:var(--accent);text-shadow:0 0 12px var(--accent)}
#immersive{position:fixed;inset:0;z-index:200;background:#03070b;display:none;overflow:hidden}
#immersive.active{display:block;animation:immersiveIn .42s ease both}
@keyframes immersiveIn{from{opacity:0;transform:scale(.98)}to{opacity:1;transform:scale(1)}}
#immersive video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;transform:scaleX(-1);opacity:.78}
#drawCanvas{position:absolute;inset:0;width:100%;height:100%;touch-action:none}
.imm-hud{position:absolute;inset:0;pointer-events:none;border:1px solid rgba(70,224,208,.22);box-shadow:inset 0 0 100px rgba(70,224,208,.08)}
.imm-top{position:absolute;left:22px;right:22px;top:18px;display:flex;justify-content:space-between;color:#fff;font-size:11px;letter-spacing:.14em}
.imm-controls{position:absolute;bottom:22px;left:50%;transform:translateX(-50%);display:flex;gap:8px;align-items:center}
.imm-controls button{border:1px solid rgba(255,255,255,.16);background:rgba(4,8,12,.65);color:#fff;border-radius:12px;padding:9px 12px;backdrop-filter:blur(14px);cursor:pointer}
#immPointer{position:absolute;width:15px;height:15px;border:2px solid var(--accent);border-radius:50%;box-shadow:0 0 16px var(--accent);pointer-events:none;display:none;z-index:4}
#visionReadout{position:absolute;right:20px;bottom:76px;max-width:340px;padding:12px;border-radius:14px;background:rgba(4,8,12,.7);border:1px solid rgba(70,224,208,.25);backdrop-filter:blur(14px);font-size:12px;display:none}
.proactive{position:fixed;left:50%;bottom:110px;transform:translateX(-50%);z-index:55;padding:10px 14px;border-radius:14px;background:rgba(5,10,15,.78);border:1px solid rgba(70,224,208,.22);color:#fff;font-size:12px;backdrop-filter:blur(16px);opacity:0;pointer-events:none;transition:.3s}
.proactive.show{opacity:1}
@media(max-width:700px){#coreHud{width:70vw}.tool{font-size:8px}#activityPanel{top:auto;bottom:118px;right:12px}.imm-controls{bottom:12px}}
#bootSplash{position:fixed;inset:0;z-index:95;display:flex;align-items:center;justify-content:center;background:#03070b;transition:opacity .65s ease,visibility .65s}.bootCore{width:110px;height:110px;border-radius:50%;border:1px solid color-mix(in srgb,var(--accent) 60%,transparent);box-shadow:0 0 45px color-mix(in srgb,var(--accent) 35%,transparent),inset 0 0 35px color-mix(in srgb,var(--accent) 12%,transparent);display:grid;place-items:center;animation:coreBreath 1.8s ease-in-out infinite}.bootCore:after{content:"JARVIS";font-size:10px;letter-spacing:.28em;margin-left:.28em}.bootDone{opacity:0;visibility:hidden}</style></head><body>
<div id="scene"></div><div id="bootSplash"><div class="bootCore"></div></div><div id="coreHud" aria-hidden="true"><div class="core-ring"></div><div class="core-ring r2"></div><div class="core-ring r3"></div><div class="core-wave"></div><div class="core-energy"></div></div>
<div id="activityPanel" class="glass"><div class="title">JARVIS ACTIVITY</div><div id="activityList"></div></div>
<div id="toolBar"><span class="tool" data-tool="GROQ">● GROQ</span><span class="tool" data-tool="MEMORY">● MEMORY</span><span class="tool" data-tool="WEB">● WEB</span><span class="tool" data-tool="VISION">● VISION</span><span class="tool" data-tool="MIC">● MIC</span></div>
<div id="proactiveTip" class="proactive"></div>
<div id="immersive"><video id="immVideo" playsinline muted></video><canvas id="drawCanvas"></canvas><div class="imm-hud"></div><div class="imm-top"><span>JARVIS / IMMERSIVE</span><span id="immStatus">VISION ACTIVE · HAND TRACKING</span></div><div id="immPointer"></div><div id="visionReadout"></div><div class="imm-controls"><button id="immVision">Ask Jarvis what I see</button><button id="immClear">Clear</button><button id="immExit">Exit</button></div></div><div id="top"><div class="brand"><i></i>JARVIS</div><div class="status" id="status">INITIALIZING</div></div><div class="tip" id="tip">Move your hand • pinch to select • open palm to zoom</div><video id="handcam" playsinline></video><div id="pointer"></div>
...
function speakJarvis(text,callback){
 ...
 function finishSpeech(){
  isJarvisSpeaking=false;setCoreState('idle');if(jarvis)jarvis.userData.talking=false;
 ...
 function speakNext(){
  ...
  utter.onstart=()=>{isJarvisSpeaking=true;setCoreState('speaking');activityAdd('JARVIS','Speaking');if(jarvis)jarvis.userData.talking=true;beginSyntheticVoiceMeter(spoken);showVoiceStatus('JARVIS SPEAKING')};
 ...
 if(localStorage.getItem('jarvisVoiceMode')==='elevenlabs'){
  try{
   ...
   if(r.ok){
    ...
    jarvisAudio.onplay=()=>{isJarvisSpeaking=true;setCoreState('speaking');activityAdd('JARVIS','Speaking');if(jarvis)jarvis.userData.talking=true;beginVoiceMeterFromAudio(jarvisAudio);showVoiceStatus('JARVIS SPEAKING')};
...
async function sendSilent(text){
 lastUserText=text;lastInteraction=Date.now();
 const low=text.toLowerCase();if(/weather|news|internet|search|website|web|look up/.test(low)) {activityAdd('WEB','Retrieving current information');setTool('WEB',true)}
 if(/remember|favourite|favorite|what do i|memory|saved/.test(low)) {activityAdd('MEMORY','Retrieving relevant memory');setTool('MEMORY',true)}
 const loadMatch=text.trim().match(/^(?:jarvis[, ]*)?(?:load|open)\s+(?:my\s+)?creation\s+(.+)$/i);
 if(loadMatch){await loadCreation(loadMatch[1].trim());setBusy(false);return;}
...
async 
const activity=[];
let proactiveEnabled=localStorage.getItem('jarvisProactive')!=='off';
let lastUserText='', lastInteraction=Date.now(), proactiveTimer=null;
let immersive=false, immStream=null, drawCtx=null, drawing=false, currentStroke=null, creationObjects=[], selectedStroke=null, pinchActive=false;
let snapLatch=false, snapCooldown=0, lastIndex={x:0,y:0,t:0};

function setCoreState(state){
  const el=$('coreHud'); if(!el)return;
  el.classList.remove('listening','thinking','speaking','error');
  if(state&&state!=='idle')el.classList.add(state);
}
function activityAdd(tool,text){
  const list=$('activityList'); if(!list)return;
  activity.unshift({tool,text,at:Date.now()}); activity.splice(8);
  list.innerHTML=activity.map(x=>`<div class="activity-item"><b><span class="tool-dot"></span>${x.tool}</b>${x.text}</div>`).join('');
  $('activityPanel').classList.add('show'); clearTimeout(activityAdd.timer);
  activityAdd.timer=setTimeout(()=>$('activityPanel').classList.remove('show'),4500);
}
function setTool(tool,on=true){
  document.querySelectorAll('.tool').forEach(e=>e.classList.toggle('active',on&&e.dataset.tool===tool));
}
function setBusy(b){
  $('status').textContent=b?'THINKING':'ONLINE';
  setCoreState(b?'thinking':'idle');
  if(b){activityAdd('GROQ','Processing your request');setTool('GROQ',true)}
  else document.querySelectorAll('.tool').forEach(e=>e.classList.remove('active'));
}
function resizeDraw(){const c=$('drawCanvas');if(!c)return;const d=Math.min(devicePixelRatio||1,2);c.width=innerWidth*d;c.height=innerHeight*d;c.style.width=innerWidth+'px';c.style.height=innerHeight+'px';drawCtx=c.getContext('2d');drawCtx.setTransform(d,0,0,d,0,0);redraw()}
function redraw(){if(!drawCtx)return;drawCtx.clearRect(0,0,innerWidth,innerHeight);for(const o of creationObjects){if(!o.points?.length)continue;drawCtx.strokeStyle=o.color||'#46e0d0';drawCtx.lineWidth=o.width||5;drawCtx.lineCap='round';drawCtx.lineJoin='round';drawCtx.beginPath();o.points.forEach((p,i)=>i?drawCtx.lineTo(p[0]*innerWidth,p[1]*innerHeight):drawCtx.moveTo(p[0]*innerWidth,p[1]*innerHeight));drawCtx.stroke()}}
function nearestStroke(x,y){let best=null,bd=.035;creationObjects.forEach(o=>o.points?.forEach(p=>{const d=Math.hypot(p[0]-x,p[1]-y);if(d<bd){bd=d;best=o}}));return best}
function drawAt(x,y){
  if(!drawCtx)return;
  if(!currentStroke){currentStroke={points:[[x,y]],width:5,color:getComputedStyle(document.documentElement).getPropertyValue('--accent').trim()||'#46e0d0'};creationObjects.push(currentStroke)}
  else currentStroke.points.push([x,y]);
  redraw();
}
async function enterImmersive(){
  if(immersive)return; immersive=true;$('immersive').classList.add('active');setCoreState('listening');activityAdd('VISION','Immersive mode active');setTool('VISION',true);resizeDraw();
  try{
    if(!immStream)immStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:1280},height:{ideal:720}},audio:false});
    $('immVideo').srcObject=immStream;await $('immVideo').play();
  }catch(e){$('immStatus').textContent='CAMERA UNAVAILABLE · DRAWING STILL ACTIVE'}
}
function exitImmersive(){
  if(!immersive)return;immersive=false;$('immersive').classList.remove('active');setCoreState('idle');setTool('VISION',false);$('visionReadout').style.display='none';
  if(immStream){immStream.getTracks().forEach(t=>t.stop());immStream=null}
  if(creationObjects.length)promptCreationName();
}
async function promptCreationName(){
  const name=prompt('What would you like to name your creation?');if(!name?.trim())return;
  try{const r=await fetch('/api/creations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name.trim(),objects:creationObjects})});const d=await r.json();if(!r.ok)throw Error(d.error);activityAdd('MEMORY',`Saved creation "${d.name}"`)}catch(e){activityAdd('MEMORY','Could not save the creation')}}
async function loadCreation(name){
  try{const r=await fetch('/api/creations/'+encodeURIComponent(name));const d=await r.json();if(!r.ok)throw Error(d.error);creationObjects=d.objects||[];resizeDraw();enterImmersive();activityAdd('MEMORY',`Loaded "${name}"`);return true}catch(e){activityAdd('MEMORY',e.message||'Creation not found');return false}
}
async function askLiveVision(){
  if(!immersive)return;
  const v=$('immVideo'),c=document.createElement('canvas');c.width=640;c.height=360;const ctx=c.getContext('2d');ctx.translate(c.width,0);ctx.scale(-1,1);ctx.drawImage(v,0,0,c.width,c.height);
  activityAdd('VISION','Analyzing the current live frame');setTool('VISION',true);
  try{const r=await fetch('/api/live-vision',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image:c.toDataURL('image/jpeg',.72),question:'What am I holding or showing in front of the camera? Describe only what you can actually see.'})});const d=await r.json();if(!r.ok)throw Error(d.error);const box=$('visionReadout');box.textContent=d.reply||'I could not identify the object.';box.style.display='block';setTimeout(()=>box.style.display='none',9000)}catch(e){activityAdd('VISION',e.message||'Vision request failed')}}
function detectSnap(h){
  const now=performance.now();const ix=1-h[8].x, iy=h[8].y, pinch=Math.hypot(h[8].x-h[4].x,h[8].y-h[4].y)<.055;
  const speed=lastIndex.t?Math.hypot(ix-lastIndex.x,iy-lastIndex.y)/Math.max((now-lastIndex.t)/1000,.016):0;
  lastIndex={x:ix,y:iy,t:now};
  if(now<snapCooldown)return false;
  // A snap is approximated by a quick index/thumb pinch followed by a rapid index displacement.
  if(pinch&&!snapLatch){snapLatch=true;return false}
  if(!pinch&&snapLatch&&speed>1.0){snapLatch=false;snapCooldown=now+1200;return true}
  if(!pinch)snapLatch=false;
  return false;
}
$('immExit').onclick=exitImmersive;$('immClear').onclick=()=>{creationObjects=[];redraw()};
$('immVision').onclick=askLiveVision;addEventListener('resize',resizeDraw);
$('proactiveBtn').onclick=()=>{proactiveEnabled=!proactiveEnabled;localStorage.setItem('jarvisProactive',proactiveEnabled?'on':'off');$('proactiveBtn').textContent=proactiveEnabled?'On':'Off'};
$('proactiveBtn').textContent=proactiveEnabled?'On':'Off';
function showProactiveTip(){
  if(!proactiveEnabled||!lastUserText||Date.now()-lastInteraction<70000)return;
  const t=lastUserText.toLowerCase();let tip='';
  if(/\b(code|coding|app|website|python|javascript|html)\b/.test(t))tip='Tip: keep interactive camera processing local so hand tracking stays responsive.';
  else if(/\b(study|school|exam|math|science)\b/.test(t))tip='Tip: I can break this task into a short checklist if that would help.';
  else return;
  const el=$('proactiveTip');el.textContent='JARVIS · '+tip;el.classList.add('show');activityAdd('JARVIS',tip);setTimeout(()=>el.classList.remove('show'),7000);
}
proactiveTimer=setInterval(showProactiveTip,15000);

function initHands(){let v=$('handcam'),hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});hands.setOptions({maxNumHands:1,modelComplexity:1,minDetectionConfidence:.55,minTrackingConfidence:.55});hands.onResults(res=>{if(!res.multiHandLandmarks?.length){handSeen=false;$('pointer').style.display='none';if(immersive){drawing=false;currentStroke=null;$('immPointer').style.display='none'}return}handSeen=true;$('pointer').style.display='block';let h=res.multiHandLandmarks[0],x=1-h[8].x,y=h[8].y;
   if(immersive){
     const p=$('immPointer');p.style.display='block';p.style.left=(x*100)+'%';p.style.top=(y*100)+'%';
     const pinch=Math.hypot(h[8].x-h[4].x,h[8].y-h[4].y)<.055;
     if(pinch&&!pinchActive){pinchActive=true;selectedStroke=nearestStroke(x,y);drawing=!selectedStroke}
     if(pinch&&selectedStroke){const dx=x-(selectedStroke._anchor?.[0]??x),dy=y-(selectedStroke._anchor?.[1]??y);if(!selectedStroke._anchor)selectedStroke._anchor=[x,y];selectedStroke.points=selectedStroke.points.map(pt=>[Math.max(0,Math.min(1,pt[0]+dx)),Math.max(0,Math.min(1,pt[1]+dy))]);selectedStroke._anchor=[x,y];redraw()}
     else if(pinch&&drawing)drawAt(x,y);
     if(!pinch){if(selectedStroke)delete selectedStroke._anchor;selectedStroke=null;drawing=false;pinchActive=false;currentStroke=null}
     if(detectSnap(h)){exitImmersive();return}
   }
   $('pointer').style.left=(x*100)+'%';$('pointer').style.top=(y*100)+'%';let span=Math.hypot(h[0].x-h[12].x,h[0].y-h[12].y);targetZ=THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(span,.18,.42,48,13),13,48);jarvis.position.set(0,0,0);let pinch=Math.hypot(h[8].x-h[4].x,h[8].y-h[4].y)<.055;if(pinch&&!pinchLatch){pinchLatch=true;let hit=hitAt(x,y);if(hit)selectWord(hit)}if(!pinch)pinchLatch=false});let cam=new Camera(v,{onFrame:async()=>hands.send({image:v}),width:640,height:480});cam.start()}
function animate(t){
...
function finishBoot(){const b=$('bootSplash');if(!b)return;b.classList.add('bootDone');setTimeout(()=>b.remove(),700)}
async function setup(){let r=await fetch('/api/status').catch(()=>null);if(r){let d=await r.json();if(d.configured){$('setup').classList.add('hidden');setTimeout(finishBoot,250);addMsg('j','Hello. Jarvis is online. Speak with the microphone, type below, or use hand gestures.')}}}
$('begin').onclick=async()=>{let key=$('key').value.trim();if(!key){$('err').textContent='A Groq API key is required.';return}$('begin').disabled=true;try{let r=await fetch('/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({groq_key:key,save:$('save').checked})});let d=await r.json();if(!d.ok)throw Error(d.error);$('setup').classList.add('hidden');setTimeout(finishBoot,250);addMsg('j','Jarvis initialized. Gesture control and chat are ready.')}catch(e){$('err').textContent=e.message||'Could not reach the server.';$('begin').disabled=false}};
