...
def api_time():
    ...
    return jsonify({"iso":now.isoformat(),"date":now.strftime("%A, %B %d, %Y"),"time":now.strftime("%I:%M:%S %p"),"timezone":"Asia/Singapore"})
@app.route("/api/creations",methods=["GET","POST"])
def api_creations():
    try:
        items=json.load(open(CREATIONS_FILE,"r",encoding="utf-8")) if os.path.exists(CREATIONS_FILE) else []
    except Exception: items=[]
    if request.method=="GET": return jsonify({"creations":items})
    d=request.get_json(force=True,silent=True) or {}
    item={"id":uuid.uuid4().hex,"name":str(d.get("name") or "Untitled")[:100],"data":d.get("data") or {},"created_at":datetime.datetime.utcnow().isoformat()+"Z"}
    items.append(item)
    with open(CREATIONS_FILE,"w",encoding="utf-8") as f: json.dump(items,f,ensure_ascii=False)
    return jsonify({"ok":True,"creation":item})
...
/* FUTURISTIC COMMAND CENTER ADDITIONS */
#jarvisCore{position:fixed;z-index:8;left:50%;top:50%;width:260px;height:260px;transform:translate(-50%,-50%);pointer-events:none;display:grid;place-items:center}
.core{width:118px;height:118px;border-radius:50%;background:radial-gradient(circle at 38% 34%,rgba(255,255,255,.85),var(--accent) 8%,rgba(10,30,40,.82) 44%,rgba(0,0,0,.18) 70%);box-shadow:0 0 26px color-mix(in srgb,var(--accent) 70%,transparent),0 0 90px color-mix(in srgb,var(--accent) 30%,transparent);animation:coreBreathe 4s ease-in-out infinite}
.coreRing{position:absolute;border:1px solid color-mix(in srgb,var(--accent) 70%,transparent);border-radius:50%;animation:spin 14s linear infinite}.r1{width:180px;height:180px}.r2{width:240px;height:240px;animation-direction:reverse;animation-duration:22s;border-style:dashed}
.coreState{position:absolute;bottom:-20px;font-size:10px;letter-spacing:.25em;color:var(--muted)}
body[data-core="listening"] .core{animation:listen .55s ease-in-out infinite alternate}body[data-core="thinking"] .core{background:radial-gradient(circle,#fff0b0,#ffc857 18%,rgba(40,28,8,.8) 55%);animation:spin .9s linear infinite}body[data-core="speaking"] .core{animation:speak .22s ease-in-out infinite alternate}body[data-core="error"] .core{background:radial-gradient(circle,#ffd0d8,#ff4f6d 20%,rgba(40,5,10,.85) 60%)}
@keyframes coreBreathe{50%{transform:scale(1.08);filter:brightness(1.2)}}@keyframes listen{to{transform:scale(1.15);box-shadow:0 0 80px var(--accent)}}@keyframes speak{to{transform:scale(1.2)}}@keyframes spin{to{transform:rotate(360deg)}}
#toolIndicators{position:fixed;z-index:20;bottom:18px;left:50%;transform:translateX(-50%);display:flex;gap:7px}.tool{font-size:9px;letter-spacing:.12em;padding:6px 9px;border:1px solid var(--border);border-radius:999px;background:rgba(5,12,16,.55)}.tool.active{color:var(--accent);border-color:var(--accent);box-shadow:0 0 18px color-mix(in srgb,var(--accent) 35%,transparent)}
#activityPanel{position:fixed;z-index:20;right:18px;top:70px;width:250px;padding:13px;border-radius:16px;font-size:11px}.activityTitle{letter-spacing:.16em;color:var(--muted);margin-bottom:8px}.activityItem{padding:7px 0;border-top:1px solid rgba(255,255,255,.06)}
#immersive{position:fixed;z-index:200;inset:0;background:radial-gradient(circle at center,#10242c,#020609);display:none}#immersive.active{display:block}.immersiveHud{position:absolute;inset:24px;border:1px solid var(--border);pointer-events:none}.immersiveHud:before{content:"IMMERSIVE MODE • SNAP TO EXIT";position:absolute;top:12px;left:16px;letter-spacing:.18em;font-size:10px;color:var(--muted)}#drawCanvas{position:absolute;inset:0;width:100%;height:100%;touch-action:none}#immersiveCursor{position:absolute;width:18px;height:18px;border:2px solid var(--accent);border-radius:50%;transform:translate(-50%,-50%);pointer-events:none}
#bootOverlay{position:fixed;z-index:500;inset:0;background:radial-gradient(circle,#0b1d24,#020406);display:none;place-items:center;text-align:center}.bootBox{width:min(430px,86vw)}.bootLogo{font-size:28px;letter-spacing:.35em}.bootLine{height:2px;background:rgba(255,255,255,.1);margin:25px 0;overflow:hidden}.bootLine i{display:block;height:100%;width:0;background:var(--accent);box-shadow:0 0 18px var(--accent);transition:width .6s}.bootStep{font-size:11px;letter-spacing:.18em;color:var(--muted)}
</style></head><body>
...
<div id="bootOverlay"><div class="bootBox"><div class="bootLogo">JARVIS</div><div class="bootLine"><i id="bootProgress"></i></div><div class="bootStep" id="bootStep">AWAITING CORE</div></div></div>
<div id="jarvisCore"><div class="coreRing r1"></div><div class="coreRing r2"></div><div class="core"></div><div class="coreState" id="coreState">READY</div></div>
<div id="activityPanel" class="glass"><div class="activityTitle">JARVIS ACTIVITY</div><div id="activityLog"><div class="activityItem">● Ready</div></div></div>
<div id="toolIndicators"><span class="tool" id="tool-groq">GROQ</span><span class="tool" id="tool-memory">MEMORY</span><span class="tool" id="tool-web">WEB</span><span class="tool" id="tool-vision">VISION</span><span class="tool" id="tool-mic">MIC</span></div>
<div id="immersive"><canvas id="drawCanvas"></canvas><div class="immersiveHud"></div><div id="immersiveCursor"></div></div>
<script>
const $=id=>document.getElementById(id), sceneEl=$('scene'), messages=$('messages');
function coreState(state){document.body.dataset.core=state;$('coreState').textContent=state.toUpperCase()}
function tool(name,on=true){let e=$('tool-'+name);if(e)e.classList.toggle('active',on)}
function activity(title,detail=''){let l=$('activityLog');if(!l)return;l.innerHTML=`<div class="activityItem">● <b>${title}</b>${detail?'<br><span>'+detail+'</span>':''}</div>`+l.innerHTML;l.innerHTML=l.innerHTML.slice(0,1800)}
async function boot(){let o=$('bootOverlay'),p=$('bootProgress'),t=$('bootStep');o.style.display='grid';for(const [n,label] of [[15,'CONNECTING TO JARVIS'],[35,'INITIALIZING VOICE'],[55,'CALIBRATING VISION'],[75,'LOADING MEMORY'],[100,'CORE ONLINE']]){t.textContent=label;p.style.width=n+'%';await new Promise(r=>setTimeout(r,320))}await new Promise(r=>setTimeout(r,300));o.style.display='none';coreState('ready');activity('Jarvis','Command center online')}
let immersive=false,drawCtx,drawing=false,lastPoint=null,snapFrames=0,pendingCreationName=false;
function initImmersive(){let c=$('drawCanvas');c.width=innerWidth*devicePixelRatio;c.height=innerHeight*devicePixelRatio;c.style.width=innerWidth+'px';c.style.height=innerHeight+'px';drawCtx=c.getContext('2d');drawCtx.scale(devicePixelRatio,devicePixelRatio);drawCtx.lineCap='round';drawCtx.lineJoin='round';drawCtx.lineWidth=4;drawCtx.strokeStyle=accent}
function toggleImmersive(on=!immersive){immersive=on;$('immersive').classList.toggle('active',on);tool('vision',on);activity('Immersive mode',on?'Finger drawing active':'Returned to command center');if(on)initImmersive();else if(drawing){drawing=false;pendingCreationName=true;setTimeout(()=>{addMsg('j','What would you like to name your creation?');speak('What would you like to name your creation?')},350)}}
async function saveCreation(name){let c=$('drawCanvas');let r=await fetch('/api/creations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,data:{image:c.toDataURL('image/png')}})});if(r.ok){activity('Creation saved',name);addMsg('j',`Saved your creation as “${name}”.`)}}
...
async function sendSilent(text){
 ...
 setBusy(true);
 try{
  let timeData=null;try{timeData=await fetch('/api/time').then(r=>r.json())}catch(e){} let r=await fetch('/api/message',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text,current_time:timeData,performance:{fps:fpsValue,lagging:fpsValue>0&&fpsValue<45}})});
  let d=await r.json();
  setBusy(false);
...
function setBusy(b){$('status').textContent=b?'THINKING':'ONLINE';coreState(b?'thinking':'ready');tool('groq',b);if(b)activity('GROQ','Processing request')}
async function send(){let t=$('text').value.trim();if(!t)return;if(pendingCreationName){pendingCreationName=false;$('text').value='';await saveCreation(t);return};$('text').value='';addMsg('user',t);let focus=(t.toLowerCase().match(/\b(?:about|on|for|into|regarding)\s+([a-z0-9_-]{4,})/)||[])[1]||t.toLowerCase().replace(/[^a-z0-9 ]/g,' ').split(/\s+/).find(w=>w.length>5&&!['please','could','would','jarvis','tell','about'].includes(w));rebuildWords(t,focus);await sendSilent(t)}
...
async function initJarvisVoice(){
 ...
 if(SpeechRecognition){
   ...
   speechRecognition.onstart=()=>{coreState('listening');tool('mic',true);activity('Listening','Voice input active');listening=true;$('mic').innerHTML=STOP_ICON;showVoiceStatus('JARVIS LISTENING')};
   ...
   speechRecognition.onend=()=>{tool('mic',false);listening=false;$('mic').innerHTML=MIC_ICON;const t=$('text').value.trim();if(!t){showVoiceStatus('ONLINE');return}showVoiceStatus('THINKING');send()};
...
async function setup(){let r=await fetch('/api/status').catch(()=>null);if(r){let d=await r.json();if(d.configured){$('setup').classList.add('hidden');boot();addMsg('j','Hello. Jarvis is online. Speak with the microphone, type below, or use hand gestures.')}}}
