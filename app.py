...
#jarvisImmersive.entering #jarvisImmersiveVideo{transform:scale(1.035)}
...
#jarvisImmersive video{position:fixed!important;left:0!important;top:0!important;width:100vw!important;height:100vh!important;max-width:none!important;max-height:none!important;object-fit:cover!important;object-position:center!important;opacity:.32;transform:scaleX(-1);display:block!important}
...
async function initHands(){
  ...
  hands.onResults(res=>{
    ...
    if(!window.__jarvisImmersiveActive){
...
(()=>{
 ...
 function canvasPoint(x,y){return {x:clamp(1-x,0,1),y:clamp(y,0,1)}}
 ...
 async function loadCreationNamed(name){name=(name||prompt('Which creation should I load?')||'').trim();if(!name)return;const r=await fetch('/api/creations');const d=await r.json();const found=(d.creations||[]).find(x=>String(x.name||'').toLowerCase()===name.toLowerCase());if(!found)throw Error('I could not find “'+name+'”.');strokes=Array.isArray(found.strokes)?structuredClone(found.strokes):[];redraw();activity('MEMORY','Loaded “'+found.name+'”.','memory')}
 ...
 function angle(a,b,c){const ab={x:a.x-b.x,y:a.y-b.y},cb={x:c.x-b.x,y:c.y-b.y};const den=Math.hypot(ab.x,ab.y)*Math.hypot(cb.x,cb.y);if(!den)return 0;return Math.acos(clamp((ab.x*cb.x+ab.y*cb.y)/den,-1,1))*180/Math.PI}
 function fingerCurled(h,tip,pip,mcp){
   const wrist=h[0],tipP=h[tip],pipP=h[pip],mcpP=h[mcp??(pip-1)];
   const bend=angle(mcpP,pipP,tipP);
   return bend<155 || dist(tipP,wrist)<dist(pipP,wrist)*1.08;
 }
 function fingerExtended(h,tip,pip,mcp){
   const bend=angle(h[mcp],h[pip],h[tip]);
   return bend>160 && dist(h[tip],h[0])>dist(h[pip],h[0])*1.04;
 }
 // Use the four long fingers for fist detection; thumb position varies a lot with hand orientation.
 function isFist(h){
   return fingerCurled(h,8,6,5)&&fingerCurled(h,12,10,9)&&fingerCurled(h,16,14,13)&&fingerCurled(h,20,18,17);
 }
 function isPointing(h){
   return fingerExtended(h,8,6,5)&&fingerCurled(h,12,10,9)&&fingerCurled(h,16,14,13)&&fingerCurled(h,20,18,17);
 }
 function isIndexMiddleTogether(h){return dist(h[8],h[12])<.075&&!fingerCurled(h,8,6)&&!fingerCurled(h,12,10)}
 ...
 function immersiveHands(hands){if(!hands.length){if(ip)ip.classList.remove('on');return}const h=hands[0],idx=h[8],thumb=h[4],p=canvasPoint(idx.x,idx.y);if(ip){ip.classList.add('on');ip.style.left=(p.x*100)+'%';ip.style.top=(p.y*100)+'%'}
   ...
   if(isPointing(h)){if(!drawing){drawing=true;currentStroke={color:drawColor,size:brushSize,points:[p]};strokes.push(currentStroke)}else{currentStroke.points.push(p);redraw()}}else{drawing=false;currentStroke=null}
 }
 window.jarvisHandleHands=function(input){
   const hands=Array.isArray(input?.[0])?input:[];
   if(!hands.length){
     if($('pointer'))$('pointer').style.display='none';
     fistStart=0;exitFistStart=0;
     if(countdown){countdown.classList.remove('show');countdown.textContent=''}
   ...
   window.jarvisImmersiveVision=async function(question){if(!immersive||!video?.videoWidth)throw Error('Immersive camera is not ready.');const c=document.createElement('canvas');c.width=Math.min(video.videoWidth,1280);c.height=Math.round(c.width*video.videoHeight/video.videoWidth);c.getContext('2d').drawImage(video,0,0,c.width,c.height);if(strokes.length){const ctx=c.getContext('2d');for(const st of strokes){ctx.strokeStyle=st.color;ctx.lineWidth=st.size*Math.min(devicePixelRatio||1,2);ctx.lineCap='round';ctx.beginPath();st.points.forEach((p,i)=>{const x=p.x*c.width,y=p.y*c.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke()}}const r=await fetch('/api/photo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image:c.toDataURL('image/jpeg',.8),question:question||'What do you see in this current camera frame? Be careful and only describe what is actually visible.'})});const d=await r.json();if(!r.ok)throw Error(d.error||'Vision analysis failed.');activity('VISION','Analyzed one current frame only.','vision');return d.reply};
 // Voice/text commands for immersive mode, save and load.
 const oldSend=window.send; if(typeof oldSend==='function'&&!window.__immersiveCommandWrapped){window.__immersiveCommandWrapped=true;window.send=async function(){const text=($('text')?.value||'').trim();const low=text.toLowerCase();if(/^(open|enter|start|launch)\b.*immersive/.test(low)||/immersive mode/.test(low)&&/open|enter|start|launch/.test(low)){if($('text'))$('text').value='';enter();return}if(/^(exit|close|leave|stop)\b.*immersive/.test(low)){if($('text'))$('text').value='';exit();return}if(immersive&&/save (this|my|the) creation|save .*under/.test(low)){const m=low.match(/(?:under|as|named)\s+(.+)$/i);if($('text'))$('text').value='';try{await saveCreationNamed(m?m[1]:null)}catch(e){activity('MEMORY',e.message,'memory')}return}if(immersive&&/load (my|the)?\s*creation/.test(low)){const m=low.match(/creation\s+(.+)$/i);if($('text'))$('text').value='';try{await loadCreationNamed(m?m[1]:null)}catch(e){activity('MEMORY',e.message,'memory')}return}if(immersive&&/what am i holding|what is this|what's this|what do you see|describe what you see|what did i draw|what have i drawn/i.test(low)){if($('text'))$('text').value='';try{const reply=await window.jarvisImmersiveVision(text);if(typeof addMsg==='function')addMsg('j',reply);if(typeof speakJarvis==='function')speakJarvis(reply)}catch(e){if(typeof addMsg==='function')addMsg('j',e.message)}return}return oldSend.apply(this,arguments)}}
