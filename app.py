...
def handle_command(text):
    ...
    if m:
        q=m.group(1).strip()
        try:
            rr=requests.get("https://www.bing.com/images/search",params={"q":q,"form":"HDRSC2"},headers={"User-Agent":"Mozilla/5.0"},timeout=12);imgs=[]
            for mm in re.finditer(r'"murl":"(.*?)"',rr.text):
                try:u=json.loads('"'+mm.group(1)+'"')
                except Exception:u=html.unescape(mm.group(1))
                if u.startswith("http") and u not in [x["url"] for x in imgs]:imgs.append({"url":u,"title":q})
                if len(imgs)>=6:break
