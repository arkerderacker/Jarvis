...
async function initHands(){if(window.__handsStarted)return true;window.__handsStarted=true;let v=$('handcam'),hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});hands.setOptions({maxNumHands:2,modelComplexity:1,minDetectionConfidence:.55,minTrackingConfidence:.55});hands.onResults(res=>{if(!res.multiHandLandmarks?.length){handSeen=false;$('pointer').style.display='none';if(window.jarvisHandleHands)window.jarvisHandleHands(null);return}handSeen=true;$('pointer').style.display='block';let h=res.multiHandLandmarks[0],x=1-h[8].x,y=h[8].y;if(window.jarvisHandleHands)window.jarvisHandleHands(res.multiHandLandmarks);$('pointer').style.left=(x*100)+'%';$('pointer').style.top=(y*100)+'%';let span=Math.hypot(h[0].x-h[12].x,h[0].y-h[12].y);targetZ=THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(span,.18,.42,48,13),13,48);jarvis.position.set(0,0,0);let pinch=Math.hypot(h[8].x-h[4].x,h[8].y-h[4].y)<.055;if(pinch&&!pinchLatch){pinchLatch=true;let hit=hitAt(x,y);if(hit)selectWord(hit)}if(!pinch)pinchLatch=false});let cam=new Camera(v,{onFrame:async()=>hands.send({image:v}),width:640,height:480});try{if(navigator.mediaDevices?.getUserMedia){const stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480}},audio:false});stream.getTracks().forEach(t=>t.stop())}}catch(e){window.__handsStarted=false;throw Error('Camera permission is required for hand tracking.')}cam.start();return true}
function animate(t){
...
async function startHandsAfterInit(){try{await initHands();$('status').textContent='READY';if(window.jarvisActivity)window.jarvisActivity('VISION','Hand tracking ready.','vision')}catch(e){$('status').textContent='CAMERA ERROR';$('err').textContent=e.message||'Camera permission is required for hand tracking.';throw e}}
async function setup(){let r=await fetch('/api/status').catch(()=>null);if(r){let d=await r.json();if(d.configured){try{await startHandsAfterInit();$('setup').classList.add('hidden');addMsg('j','Hello. Jarvis is online. Speak with the microphone, type below, or use hand gestures.')}catch(e){$('setup').classList.remove('hidden')}}}}
$('begin').onclick=async()=>{let key=$('key').value.trim();if(!key){$('err').textContent='A Groq API key is required.';return}$('begin').disabled=true;$('err').textContent='Connecting to Jarvis…';try{let r=await fetch('/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({groq_key:key,save:$('save').checked})});let d=await r.json();if(!d.ok)throw Error(d.error);$('err').textContent='Groq connected. Requesting camera permission…';await startHandsAfterInit();$('setup').classList.add('hidden');addMsg('j','Jarvis initialized. Gesture control and chat are ready.')}catch(e){$('err').textContent=e.message||'Could not initialize Jarvis.';$('begin').disabled=false}};
