...
async function initHands(){
  ...
  hands.onResults(res=>{
    ...
    handSeen=true;$('pointer').style.display='block';
    ...
    if(!window.__jarvisImmersiveActive)targetZ=THREE.MathUtils.clamp(THREE.MathUtils.mapLinear(span,.18,.42,48,13),13,48);
...
<div id="fistCountdown" aria-hidden="true"></div><div id="jarvisImmersive" aria-hidden="true">
 ...
 <div class="jiMenu">
   ...
   <button id="jarvisUndo">Undo</button><button id="jarvisClear">Clear</button><button id="jarvisSave">Save</button><button id="jarvisLoad">Load</button><button id="jarvisExit">Exit</button>
 </div>
</div>
...
(()=>{
 ...
 let strokes=[], drawing=false, currentStroke=null, selectedStroke=null, lastPinch=null, resizeStart=null, resizeBase=null, panelDrag=null;
 ...
 function canvasPoint(x,y){return {x:clamp(x,0,1),y:clamp(y,0,1)}}
 ...
 function enter(){if(immersive||performance.now()-lastToggle<1200)return;
   immersive=true;window.__jarvisImmersiveActive=true;lastToggle=performance.now();fistStart=0;
   // Freeze/reset the normal 3D camera so hand zoom cannot continue underneath immersive mode.
   ...
   imm.classList.add('open','entering');imm.setAttribute('aria-hidden','false');document.body.classList.add('jarvis-immersive-active');if(typeof setChatOpen==='function')setChatOpen(true);
 ...
 function exit(){if(!immersive||performance.now()-lastToggle<700)return;immersive=false;window.__jarvisImmersiveActive=false;lastToggle=performance.now();imm.classList.remove('open','entering');imm.setAttribute('aria-hidden','true');document.body.classList.remove('jarvis-immersive-active');drawing=false;currentStroke=null;selectedStroke=null;lastPinch=null;resizeStart=null;resizeBase=null;ip?.classList.remove('on');
 ...
 $('jarvisBrush')?.addEventListener('input',e=>{brushSize=Number(e.target.value)||4;$('jarvisBrushValue').textContent=brushSize});
 ...
 function fingerExtended(h,mcp,pip,tip){return fingerAngle(h,mcp,pip,tip)>150}
 function fingerCurled(h,mcp,pip,tip){return fingerAngle(h,mcp,pip,tip)<135}
 function isFist(h){
   // Robust closed-fist test: curled finger joints + fingertips tucked toward the palm.
   // A brief tracking imperfection should not prevent the 5-second entry/exit gesture.
   const curled=[fingerCurled(h,5,6,8),fingerCurled(h,9,10,12),fingerCurled(h,13,14,16),fingerCurled(h,17,18,20)];
   const curledCount=curled.filter(Boolean).length;
   const palm=Math.hypot(h[0].x-h[9].x,h[0].y-h[9].y)||.001;
   const tucked=[8,12,16,20].filter(i=>dist(h[i],h[9])<palm*2.25).length;
   const thumbAcross=dist(h[4],h[9])<palm*1.55 || dist(h[4],h[5])<palm*1.35;
   return curledCount>=3 && tucked>=3 && thumbAcross;
 }
 ...
 function immersiveHands(hands){if(!hands.length){if(ip)ip.classList.remove('on');return}const h=hands[0],idx=h[8],thumb=h[4],p=canvasPoint(idx.x,idx.y);if(ip){ip.classList.add('on');ip.style.left=(p.x*100)+'%';ip.style.top=(p.y*100)+'%'}
   const pinch=dist(idx,thumb)<.07;
   if(hands.length>=2){const h2=hands[1],a=dist(h[8],h[4]),b=dist(h2[8],h2[4]),twoDist=dist(h[8],h2[8]);if(pinch&&dist(h2[8],h2[4])<.07){if(!resizeStart){resizeStart=twoDist;resizeBase=selectedStroke||strokeHit(p)}else if(resizeBase){const factor=clamp(twoDist/Math.max(resizeStart,.05),.35,3);const cx=resizeBase.points.reduce((a,q)=>a+q.x,0)/resizeBase.points.length,cy=resizeBase.points.reduce((a,q)=>a+q.y,0)/resizeBase.points.length;resizeBase.points.forEach(q=>{q.x=clamp(cx+(q.x-cx)*factor,0,1);q.y=clamp(cy+(q.y-cy)*factor,0,1)});redraw()}return}else{resizeStart=null;resizeBase=null}}
   if(pinch){if(!selectedStroke)selectedStroke=strokeHit(p);if(selectedStroke){if(!lastPinch)lastPinch=p;const dx=p.x-lastPinch.x,dy=p.y-lastPinch.y;selectedStroke.points.forEach(q=>{q.x=clamp(q.x+dx,0,1);q.y=clamp(q.y+dy,0,1)});lastPinch=p;redraw()}return}
   ...
   }else{drawing=false;currentStroke=null}
 }
