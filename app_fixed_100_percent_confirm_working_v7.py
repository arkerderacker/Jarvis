...
#jarvisImmersive video{position:fixed!important;left:0!important;top:0!important;width:100vw!important;height:100vh!important;max-width:none!important;max-height:none!important;object-fit:cover!important;object-position:center!important;opacity:.32;transform:none!important;display:block!important}
...
(()=>{
 ...
 let strokes=[], drawing=false, currentStroke=null, selectedStroke=null, lastPinch=null, resizeStart=null, resizeBase=null, panelDrag=null, autoDraw=localStorage.getItem('jarvisAutoDraw')==='1', autoDrawTimer=0;
 ...
 function enter(){if(immersive||performance.now()-lastToggle<1200)return;
   ...
   const src=$('handcam');if(src?.srcObject){video.srcObject=src.srcObject;video.play?.().catch(()=>{})}
 ...
 function isFist(h){
   ...
   return !indexExtended && curledCount>=3 && tucked>=3; 
 }
 ...
 async function autoEnhanceDrawing(){
   ...
   autoDrawTimer=setTimeout(async()=>{
     try{
       const image=canvas.toDataURL('image/png'); // transparent drawing canvas ONLY; camera video is never composited.
 ...
 function immersiveHands(hands){
   if(!hands.length){if(ip)ip.classList.remove('on');drawing=false;currentStroke=null;return}
   ...
   if(pinch){
     ...
     if(selectedStroke){if(!lastPinch)lastPinch=p;const dx=p.x-lastPinch.x,dy=p.y-lastPinch.y;selectedStroke.points.forEach(q=>{q.x=clamp(q.x+dx,0,1);q.y=clamp(q.y+dy,0,1)});lastPinch=p;redraw()}
     return;
   }
   lastPinch=null;selectedStroke=null;
   if(isPointing(h)){
     if(!drawing){drawing=true;currentStroke={color:drawColor,size:brushSize,points:[p,{x:p.x+0.0001,y:p.y+0.0001}]};strokes.push(currentStroke);redraw()}
     else{currentStroke.points.push(p);redraw()}
   }else if(drawing){
     drawing=false;currentStroke=null;autoEnhanceDrawing();
   }
 }
 window.jarvisHandleHands=function(input){
   ...
   window.jarvisImmersiveVision=async function(question,explicitRequest=false){if(!explicitRequest)throw Error('Vision only runs when you explicitly ask Jarvis to analyze the current immersive camera view.');if(!immersive||!video?.videoWidth)throw Error('Immersive camera is not ready.');const c=document.createElement('canvas');c.width=Math.min(video.videoWidth,1280);c.height=Math.round(c.width*video.videoHeight/video.videoWidth);c.getContext('2d').drawImage(video,0,0,c.width,c.height);if(strokes.length){const ctx=c.getContext('2d');for(const st of strokes){ctx.strokeStyle=st.color;ctx.lineWidth=st.size*Math.min(devicePixelRatio||1,2);ctx.lineCap='round';ctx.beginPath();st.points.forEach((p,i)=>{const x=p.x*c.width,y=p.y*c.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke()}}const r=await fetch('/api/photo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image:c.toDataURL('image/jpeg',.8),question:question||'What do you see in this current camera frame? Be careful and only describe what is actually visible.'})});const d=await r.json();if(!r.ok)throw Error(d.error||'Vision analysis failed.');activity('VISION','Analyzed one current frame only.','vision');return d.reply};
